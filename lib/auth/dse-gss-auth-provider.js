 ...
 * on the IP Address.
 * You can use {@link DseGssAuthProvider.reverseDnsResolver} if you use the FQDN for the principal.
 ...
 * @constructor
 */
function DseGssAuthProvider(gssOptions) {
  ...
  this.hostNameResolver = gssOptions.hostNameResolver || DseGssAuthProvider.useIpResolver;
}
...
DseGssAuthProvider.prototype.newAuthenticator = function (endpoint, name) {
  ...
  return new GssAuthenticator(this._kerberos, address, name, this.user, this.service, this.hostNameResolver);
};

DseGssAuthProvider.reverseDnsResolver = function (ip, callback) {
...
DseGssAuthProvider.useIpResolver = function (ip, callback) {
  callback(null, ip);
};
 ...
 * @private
 */
function GssAuthenticator(kerberosModule, address, authenticatorName, user, service, hostNameResolver) {
  BaseDseAuthenticator.call(this, authenticatorName);
  this.user = user;
  this.address = address;
  this.client = GssClient.createNew(kerberosModule, user, service);
