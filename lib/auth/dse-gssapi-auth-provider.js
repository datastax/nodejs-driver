...
/**
 * @class
 * @classdesc AuthProvider that provides GSSAPI authenticator instances for clients to connect
...
/**
 * @class
 * @classdesc Authenticator for GSSAPI.
...
class GssapiAuthenticator extends BaseDseAuthenticator {
   ...
   */
  constructor(kerberosModule, address, authenticatorName, authorizationId, service, hostNameResolver) {
  ...
  initialResponse(callback) {
    ...
    utils.series([
      (next) => {
        this.hostNameResolver(this.address, (err, name) => {
          if (!err && name) {
            host = name;
          }
          next();
      ...
      (next) => {
        this.client.init(host, (err) => {
          if (err) {
            return next(err);
          }
          if (!this._isDseAuthenticator()) {
            return this.evaluateChallenge(this.getInitialServerChallenge(), next);
          }
          next(null, this.getMechanism());
        });
      }
    ], callback);
  }
