 ...
 * @param {Boolean} [allowContinuousPaging]
 ...
 */
function createQueryOptions(client, userOptions, rowCallback, logged, allowContinuousPaging) {
  var profile =
  ...
  var result = {
    ...
    continuousPaging: ifUndefined(
      userOptions.continuousPaging, allowContinuousPaging && defaultQueryOptions.continuousPaging),
    customPayload: createCustomPayload(userOptions, defaultQueryOptions),
  ...
  while (i--) {
    ...
    if (result[key] !== undefined) {
      // It was already set with the fixed properties
      continue;
    }
...
/**
 * Creates a payload for given user and default query options.
 * @param {QueryOptions} userOptions
 * @param {QueryOptions} defaultQueryOptions
 * @private
 */
function createCustomPayload(userOptions, defaultQueryOptions) {
  var customPayload = ifUndefined(userOptions.customPayload, defaultQueryOptions.customPayload);
  var executeAs = userOptions.executeAs || defaultQueryOptions.executeAs;
  if (executeAs) {
    // We could introduce a small string/buffer cache to reuse allocations in the future
    if (!customPayload) {
      customPayload = {};
      customPayload[proxyExecuteKey] = utils.allocBufferFromString(executeAs);
    }
    else if (!customPayload[proxyExecuteKey]) {
      // Avoid appending to the user's payload object
