...
Client.prototype._prepareQuery = function (query, handler, callback) {
  ...
  var response = this.metadata.preparedQueries[name];
  if (response) {
  ...
  handler.send(
    ...
    function announcePrepared(err, response) {
      if (!err) {
        self.metadata.preparedQueries[name] = response;
        self.metadata.preparedQueriesCount++;
      }
...
Client.prototype._truncatePrepared = function () {
  //overflow protection
  var prepared = this.metadata.preparedQueries;
  var count = this.metadata.preparedQueriesCount + this._preparingCount;
  if (count >= this.options.maxPrepared) {
    ...
    if (toRemove) {
      delete prepared[toRemove];
      this.metadata.preparedQueriesCount--;
    }
  }
};
