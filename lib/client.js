...
const DefaultMetrics = require('./metrics/default-metrics');
...
function Client(options) {
   ...
   * @type {ClientMetrics}
   */
  this.metrics = options.metrics || new DefaultMetrics(); //TODO: Define default and validate
}
...
Client.prototype._batchCb = function (queries, options, callback) {
  ...
  utils.series([
    ...
    next => this._createBatchRequest(queryItems, options, (err, r) => {
      request = r;
      next(err);
    }),
    next => RequestHandler.send(request, options, this, next)
  ], callback);
};
...
Client.prototype._innerExecute = function (query, params, options, callback) {
  ...
  utils.series([
    ...
    next => {
      if (!options.prepare) {
        return this._createQueryRequest(query, options, params, (err, r) => {
          request = r;
          next(err);
        });
      }
      ...
      PrepareHandler.getPrepared(this, lbp, query, this.keyspace, (err, queryId, meta) => {
        ...
        this._createExecuteRequest(query, queryId, options, params, meta, (err, r) => {
          request = r;
          next(err);
        });
      });
 ...
 * @private
 */
Client.prototype._createBatchRequest = function (queryItems, options, callback) {
  const firstQuery = queryItems[0];
  if (!firstQuery.meta) {
    return callback(null, new requests.BatchRequest(queryItems, options));
  }

  this._setRoutingInfo(options, firstQuery.params, firstQuery.meta, function (err) {
    if (err) {
      return callback(err);
    }
    callback(null, new requests.BatchRequest(queryItems, options));
  });
};
 ...
 * @private
 */
Client.prototype._createExecuteRequest = function(query, queryId, options, params, meta, callback) {
  try {
  ...
  this._setRoutingInfo(options, params, meta, err => {
    if (err) {
      return callback(err);
    }
    callback(null, new requests.ExecuteRequest(query, queryId, params, options, meta));
  });
};
 ...
 * @private
 */
Client.prototype._createQueryRequest = function (query, options, params, callback) {
  this.metadata.adaptUserHints(this.keyspace, options.hints, (err) => {
    if (err) {
      return callback(err);
    }
    let paramsInfo;
    try {
      ...
      this._getEncoder().setRoutingKeyFromUser(paramsInfo.params, options, paramsInfo.keyIndexes);
    }
    catch (err) {
      return callback(err);
    }
    callback(null, new requests.QueryRequest(query, paramsInfo.params, options, paramsInfo.namedParameters));
  });
};
 ...
 * @private
 */
Client.prototype._setRoutingInfo = function (options, params, meta, callback) {
  const self = this;
  ...
  function encodeRoutingKey(fromUser) {
    ...
    try {
      if (fromUser) {
        encoder.setRoutingKeyFromUser(params, options);
      } else {
        encoder.setRoutingKeyFromMeta(meta, params, options);
      }
    }
    catch (err) {
      return callback(err);
