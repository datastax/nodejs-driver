...
/**
 * Prepare one or more queries. The response(s) from Cassandra are
 * passed to the callback,
 * but are usually ignored because the prepared queries are stored in the client. This
 * method does not replace the {@link QueryOptions}.prepare flag.
 * @param {String|Array.<String>} queries Query or queries to prepare with ? or :param_name as parameter placeholders
 * @param {function} callback Executes callback(err, res) when there is a prepared statement on a connection or there is an error.
 */
Client.prototype.prepare = function (queries, callback) {
  var prepare = ('string' === typeof queries) ? this._prepareQuery : this._prepareQueries;
  var self = this;
  this.connect(function afterConnect(err) {
    prepare.call(self, queries, new RequestHandler(self, self.options), callback);
  });
};

/**
 * Clear the prepared queries. Queries currently being prepared will not be removed.
 */
Client.prototype.clearPrepared = function () {
  this._prepared = {};
  this._preparedCount = 0;
};

/**
 * Prepare one query with handler.
 * If the query is currently being prepared, it queues the callback.
 * @param {String} query Query to prepare with ? or :param_name as parameter placeholders
 * @param {RequestHandler} handler
 * @param {function} callback Executes callback(err, res) when there is a prepared statement on a connection or there is an error.
 */
Client.prototype._prepareQuery = function (query, handler, callback) {
  this._truncatePrepared(); //overflow protection.
  var name = ( this.keyspace || '' ) + query;
  var response = this._prepared[name];
  if (response) {
    return setImmediate(function() {
      callback(null, response);
    });
  }
  this.once('prepared:' + name, callback);
  if (this._preparing[name]) {
    return;
  }
  this._preparing[name] = true;
  this._preparingCount++;
  var self = this;
  handler.send(
    new requests.PrepareRequest(query),
    null,
    function announcePrepared(err, response) {
      if (!err) {
        self._prepared[name] = response;
        self._preparedCount++;
      }
      delete self._preparing[name];
      self._preparingCount--;
      self.emit('prepared:' + name, err, response);
    }
  );
};

Client.prototype._prepareQueries = function (queries, handler, callback) {
  var self = this;
  async.mapSeries(
    queries,
    function (query, next) {
      self._prepareQuery(query, handler, next);
    },
    callback
  );
};
...
/**
 * Remove a query from the prepared cache if the total number of prepared and
 * preparing queries exceeds the maximum allowed.
 * @private
 */
Client.prototype._truncatePrepared = function () {
  //overflow protection
  var count = this._preparedCount + this._preparingCount;
  if (count >= this.options.maxPrepared) {
    var toRemove;
    this.log('warning', 'Prepared statements exceeded maximum. This could be caused by _preparing queries that contain parameters');
    for (var key in this._prepared) {
      if (this._prepared.hasOwnProperty(key)) {
        toRemove = key;
        break;
      }
    }
    if (toRemove) {
      delete this._prepared[toRemove];
      this._preparedCount--;
    }
  }
};
...
Client.prototype._executeAsPrepared = function (query, params, options, callback) {
  ...
  var self = this;
  async.waterfall([
    this._prepareQuery.bind(this, query, handler),
    function sendingExecute(response, next) {
 ...
 * @param {Array.<{query, params}>} queries The queries to execute as an Array of strings or as an array of object containing the query and params
 * @param {QueryOptions} [options]
 * @param {ResultCallback} callback Executes callback(err, result) when the batch was executed
 * @private
 */
Client.prototype._batchPrepared = function (queries, options, callback) {
  ...
  this._prepareQueries(
    queries.map(function (query) {return query.query}),
    handler,
    function executeBatch(err, prepareResponses) {
      if (err) return callback(err);
      var preparedQueries = prepareResponses.map(function (response, index) {
        return {
          info: { queryId: response.id, meta: response.meta },
          query: queries[index].query,
          params: queries[index].params
        };
      });
      handler.send(
        new requests.BatchRequest(preparedQueries, options),
        options,
        callback
      );
    }
