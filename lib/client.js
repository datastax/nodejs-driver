...
Client.prototype._innerExecute = function (query, params, options, callback) {
  ...
  async.series([
    ...
    function settingOptions(next) {
      self._setQueryOptions(options, params, null, null, next);
    },
  ...
  ], function innerCallback(err, results) {
...
Client.prototype._executeAsPrepared = function (query, params, options, callback) {
  ...
  async.waterfall([
    ...
    function settingOptions(queryId, meta, next) {
      var paramsInfo;
      try {
        paramsInfo = utils.adaptNamedParams(params, meta.columns);
      }
      catch (err) {
        return next(err);
      }
      params = paramsInfo.params;
      self._setQueryOptions(options, params, meta, paramsInfo.keys, function (err) {
        next(err, queryId, meta)
      });
    },
 ...
 * @private
 */
Client.prototype._setQueryOptions = function (options, params, meta, keys, callback) {
  if (!options.prepare && params && !util.isArray(params)) {
    //Named params are not supported for non-prepared statements
    //Only C* 2.1+ supports it but the driver doesn't yet
    return callback(new errors.ArgumentError('Named parameters for simple statements is not supported, use prepare flag'));
  }
  var self = this;
  async.series([
    function adaptHints(next) {
      if (options.prepare) {
        //Add the type information provided by the prepared metadata
        options.hints = meta.columns.map(function (c) { return c.type; });
        return next();
      }
      if (util.isArray(options.hints)) {
        return self.metadata.adaptUserHints(self.keyspace, options.hints, next);
      }
      next();
    },
    function adaptPageStateAndRouting(next) {
      try {
