...
function Client(options) {
  ...
  this._graphExecutor = new GraphExecutor(this, options, this._innerExecute);
}
...
Client.prototype.connect = function (callback) {
  ...
  promiseUtils.toCallback(this._connect(), callback);
};
...
Client.prototype._connect = async function () {
  ...
  try {
    await cloud.initAsync(this.options);
    ...
    if (this.options.pooling.warmup) {
      await this._warmup();
    }
...
Client.prototype.execute = function (query, params, options, callback) {
  // This method acts as a wrapper for the async method innerExecute()
  ...
  if (typeof callback === 'function') {
    ...
    return promiseUtils.toCallback(this._innerExecuteAsync(query, params, execOptions), callback);
  }
  ...
  return this._innerExecuteAsync(query, params, execOptions);
};
...
Client.prototype.executeGraph = function (query, parameters, options, callback) {
  ...
  return utils.promiseWrapper.call(this, this.options, callback, function handler(cb) {
    this._graphExecutor.send(query, parameters, options, cb);
  });
};
...
Client.prototype.eachRow = function (query, params, options, rowCallback, callback) {
  ...
  const nextPage = () => promiseUtils.toCallback(this._innerExecuteAsync(query, params, execOptions), pageCallback);
  ...
  promiseUtils.toCallback(this._innerExecuteAsync(query, params, execOptions), pageCallback);
};
...
Client.prototype.batch = function (queries, options, callback) {
  ...
  const promise = this._batch(queries, options);

  if (typeof callback === 'function') {
    return promiseUtils.toCallback(promise, callback);
  }
...
Client.prototype._batch = async function (queries, options) {
  ...
  return await RequestHandler.sendAsync(request, execOptions, this);
};
...
//TODO: Remove counter part
Client.prototype._innerExecuteAsync = async function (query, params, execOptions) {
  const version = this.controlConnection.protocolVersion;
  ...
  if (!execOptions.isPrepared()) {
    request = await this._createQueryRequestAsync(query, execOptions, params);
  } else {
    const lbp = execOptions.getLoadBalancingPolicy();
    ...
    const { queryId, meta } = await PrepareHandler.getPreparedAsync(this, lbp, query, queryKeyspace);
    request = await this._createExecuteRequestAsync(query, queryId, execOptions, params, meta);
  }
...
Client.prototype._warmup = function () {
  ...
  return promiseUtils.times(hosts.length, warmupLimit, async (index) => {
    ...
    try {
      ...
      this.log('warning', util.format('Connection pool to host %s could not be created: %s', h.address, err));
    }
  });
};
...
Client.prototype._createBatchRequest = async function (queryItems, info) {
  ...
  await this._setRoutingInfoAsync(info, firstQuery.params, firstQuery.meta);
  return new requests.BatchRequest(queryItems, info);
};
...
Client.prototype._createExecuteRequestAsync = async function(query, queryId, info, params, meta) {
  params = utils.adaptNamedParamsPrepared(params, meta.columns);
  await this._setRoutingInfoAsync(info, params, meta);
...
//TODO: Remove counterpart
Client.prototype._createQueryRequestAsync = async function (query, execOptions, params, callback) {
  await this.metadata.adaptUserHintsAsync(this.keyspace, execOptions.getHints());
...
//TODO: Remove counterpart
Client.prototype._setRoutingInfoAsync = async function (execOptions, params, meta) {
