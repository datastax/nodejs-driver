...
Client.prototype.prepare = function (query, callback) {
  var self = this;
  this.connect(function afterConnect(err) {
    self._prepare(query, new RequestHandler(self, self.options), callback);
  });
};
...
/**
 * Prepare a single query using the given handler.
 * @private
 */
Client.prototype._prepare = function (query, handler, callback) {
...
Client.prototype._executeAsPrepared = function (query, params, options, callback) {
  ...
  var self = this;
  async.waterfall([
    this._prepare.bind(this, query, handler),
...
Client.prototype._batchPrepared = function (queries, options, callback) {
  ...
  async.mapSeries(
    queries,
    function prepareQuery(query, next) {
      self._prepare(query.query, handler, function (err, response) {
        if (!err) {
          query = {
    ...
    function executeBatch(err, preparedQueries) {
 ...
 * @private
 */
Client.prototype._waitForPendingPrepares = function (queriesInfo, callback) {
  var pendingIO = false;
  var toPrepare = [];
  var self = this;
  async.each(queriesInfo, function waitIterator(item, next) {
    if (item.info.queryId) {
      //Its already prepared
      return next();
    }
    if (item.info.preparing) {
      //it is already being prepared
      pendingIO = true;
      return item.info.once('prepared', next);
    }
    toPrepare.push(item);
    next();
    ...
    if (pendingIO) {
      ...
      return setImmediate(function () {
        self._waitForPendingPrepares(toPrepare, callback);
      });
    }
