 ...
 * @property {Object|Boolean} [continuousPaging] When set, it enables the server to push the query results to the client
 * asynchronously, without the client having to request each single page. Only supported with DSE 5.1+.
 * <p>
 *   This setting is only considered by the [Client#stream()]{@link Client#stream} method. For more information,
 *   check the
 *   [paging results documentation]{@link http://docs.datastax.com/en/developer/nodejs-driver/latest/features/paging/}.
 * </p>
 * <p>
 *   Note that the properties of this setting won't inherit the values from the default query options, if you want
 *   to use a different value than the defaults defined by the driver, you must specify them each time.
 * </p>
 * @property {Number} [continuousPaging.pageSize] Defines the page size, expressed in rows or bytes, according to
 * <code>pageUnit</code>.
 * <p>Default: <code>5000</code>.</p>
 * @property {String} [continuousPaging.pageUnit] Defines the page unit, possible values are <code>'rows'</code>
 * (default) and <code>'bytes'</code>.
 * @property {Number} [continuousPaging.maxPages] Determines the maximum amount of pages to retrieve.
 * <p>Default: <code>undefined</code> (unlimited).</p>
 * @property {Number} [continuousPaging.maxPagesPerSecond] Determines the maximum amount of pages per second for the
 * server to push.
 * <p>Default: <code>undefined</code> (unlimited).</p>
 * @property {Number} [continuousPaging.highWaterMarkRows] Defines the maximum amount of rows that can be buffered
 * into the stream before pausing reads from the socket.
...
Client.prototype.stream = function (query, params, options, callback) {
  ...
  var ensureAsync = utils.ensureAsync();
  if (options.continuousPaging) {
