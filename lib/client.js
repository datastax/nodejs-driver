  ...
  async.waterfall([
    ...
    function (queryId, meta, next) {
      try {
        ...
        params = utils.adaptNamedParams(params, meta.columns);
        encoder.setRoutingKey(params, options);
      }
      catch (err) {
...
Client.prototype._batchPrepared = function (queries, options, callback) {
  //Prepares the queries that are not already prepared on a single host
  var queriesInfo = new Array(queries.length);
  var self = this;
  for (var i = 0; i < queries.length; i++) {
    var item = queries[i];
    if (!item) {
      return callback(new errors.ArgumentError(util.format('Invalid query for batch at index %d', i)));
    }
    ...
    queriesInfo[i] = {info: self.metadata.getPreparedInfo(query), query: query, params: item.params};
  }
  this._waitForPendingPrepares(queriesInfo, function (err, toPrepare) {
    if (err) return callback(err);
    //The ones that were being prepared are now prepared
    //We must prepare the pending
    if (toPrepare.length === 0) {
      return self._executePreparedBatch(queriesInfo, options, callback);
    }
    var requestsArray = new Array(toPrepare.length);
    var callbacksArray = new Array(toPrepare.length);
    toPrepare.forEach(function (item, i) {
      var info = item.info;
      info.preparing = true;
      requestsArray[i] = new requests.PrepareRequest(item.query);
    ...
    var handler = new RequestHandler(self, self.options);
    handler.sendMultiple(requestsArray, callbacksArray, function (err) {
      if (err) return callback(err);
      return self._executePreparedBatch(queriesInfo, options, callback);
    });
  });
...
/** @private */
Client.prototype._executePreparedBatch = function (queriesInfo, options, callback) {
  var request = new requests.BatchRequest(queriesInfo, options.consistency, options);
  var handler = new RequestHandler(this, this.options);
  handler.send(request, null, callback);
};
...
Client.prototype._waitForPendingPrepares = function (queriesInfo, callback) {
  ...
  var toPrepare = [];
  var self = this;
  async.each(queriesInfo, function (item, next) {
