  ...
  async.waterfall([
    ...
    function sendingExecute(queryId, meta, next) {
      try {
        ...
        self._setQueryOptions(options, params, meta);
      }
      catch (err) {
 ...
 * @private
 */
Client.prototype._setQueryOptions = function (options, params, meta) {
  if (options.prepare) {
    //Add the type information provided by the prepared metadata
    options = utils.extend(options, { hints: utils.parseColumnDefinitions(meta.columns)});
  }
  else {
  ...
  this._getEncoder().setRoutingKey(params, options);
};
