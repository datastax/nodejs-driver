...
Client.prototype._getAnalyticsMaster = function (callback) {
  this.execute('CALL DseClientTool.getAnalyticsGraphServer()', null, null, (err, result) => {
    if (err) {
      this.log('verbose', 'Error querying graph analytics server, query will not be routed optimally', err);
      return callback(null);
    }

    if (result.rows.length === 0) {
      this.log('verbose', 'Empty response querying graph analytics server, query will not be routed optimally');
      return callback(null);
    }
    ...
    const addressTranslator = this.options.policies.addressResolution;

    addressTranslator.translate(hostName, this.options.protocolOptions.port, (endpoint) =>
      callback(this.hosts.get(endpoint)));
  });
};
...
function getDefaultProfileRetryPolicy(options) {
  if (!options || !util.isArray(options.profiles)) {
    return null;
  }
