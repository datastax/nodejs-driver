...
Connection.prototype.createSocket = function() {
  ...
  var parser = new Readers.FrameParser({objectMode: true});
  ...
  utils.syncEvent([parser, this.streamingParser], 'parsingFinished', this, this.freeStreamId);
  
  parser.on('readable', function (){
    var frame = null;
    while ((frame = parser.read()) !== null) {
...
Connection.prototype.authenticate = function(callback) {
    if (!this.options.username) {
      ...
      this.sendStream(new Writers.CredentialsWriter(this.options.username, this.options.password), new ResponseHandler(callback, null, true));
    }
}
/**
...
Connection.prototype.executeToStream = function (query, params, consistency, callback) {
  query = utils.queryParser.parse(query, params);
  this.sendStream(new Writers.QueryWriter(query, consistency), new ResponseHandler(callback), true);
}
...
Connection.prototype.register = function register(events, callback) {
  this.sendStream(new Writers.RegisterWriter(events), new ResponseHandler(callback, null, true));
}

Connection.prototype.sendStream = function sendStream(frameWriter, callback, streaming) {
  if(typeof callback == 'undefined') {
    callback = function() {};
  }
