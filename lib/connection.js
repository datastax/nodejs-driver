...
Connection.prototype.handleSocketError = function (err) {
  this.emit('log', 'error', 'Socket error ' + err, err);
  if (!this.netClient.readable) {
    err.info = 'Socket error';
...
Connection.prototype.execute = function () {
  ...
  var query = utils.queryParser.parse(args.query, args.params);
  this.emit('log', 'info', 'executing query: ' + query);
  this.sendStream(new Writers.QueryWriter(query, args.consistency), args.callback);
}
...
Connection.prototype.executePrepared = function () {
  ...
  this.sendStream(new Writers.ExecuteWriter(args.query, args.params, args.consistency), args.callback, null, args.options);
}

Connection.prototype.prepare = function () {
  var args = utils.parseCommonArgs.apply(null, arguments);
  this.emit('log', 'info', 'preparing query: ' + args.query);
  this.sendStream(new Writers.PrepareQueryWriter(args.query), args.callback);
}
...
Connection.prototype.sendStream = function sendStream (frameWriter, callback, rawCallback, options) {
  this.getStreamId(function(streamId) {
    if (options && options.streamRows) {
      this.streamingParser.setStreaming(streamId, options.streamField);
      this.frameParser.ignoreFrame(streamId);
    }
    this.streamHandlers[streamId] = {callback: callback, rawCallback: rawCallback, options: options};
