...
function Connection(address, protocolVersion, options) {
  ...
  this.protocol = new streams.Protocol({objectMode: true}, this.protocolVersion);
...
Connection.prototype.bindSocketListeners = function() {
  ...
  this.netClient.pipe(this.protocol);
  ...
  return (function writeCallback (err) {
    ...
    if (frameWriter instanceof requests.ExecuteRequest || frameWriter instanceof requests.QueryRequest) {
      if (options && options.byRow) {
        self.parser.setOptions(frameWriter.streamId, { byRow: true });
      }
    }
 ...
 */
Connection.prototype.handleRow = function (header, row, meta, rowLength) {
  var streamId = header.streamId;
  if(streamId < 0) {
    return this.log('verbose', 'Event received', header);
  }
  ...
  var rowIndex = handler.rowIndex || 0;
  ...
  if(handler.callback && rowIndex === rowLength) {
    handler.callback(null, { rowLength: rowLength, meta: meta});
  }
};
