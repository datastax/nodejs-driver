...
Connection.prototype.createSocket = function() {
  ...
  self.netClient.pipe(protocol).pipe(frameParser);
  ...
  /*
  var partialReader = new Readers.PartialReader(self.netClient);
  partialReader.on('frame', self.handleFrame.bind(self));
  partialReader.on('log', function (type, info) {
    self.emit('log', type, info);
  });
  */
