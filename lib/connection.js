 ...
 * @param {Function} logEmitter
 * @param {ClientOptions} options
 * @constructor
 */
function Connection(address, protocolVersion, logEmitter, options) {
  ...
  this.logEmitter =  logEmitter || function () {};
  /** @type {ClientOptions} */
...
Connection.prototype.createSocket = function() {
  ...
  this.netClient.on('close', function() {
    self.log('verbose', 'Socket disconnected');
...
Connection.prototype.open = function (callback) {
  var self = this;
  self.log('verbose', 'Connecting to ' + this.address + ':' + this.port);
  self.createSocket();
  ...
  this.netClient.connect(this.port, this.address, function connectCallback() {
    ...
    self.sendStream(new writers.StartupWriter(), null, function (err, response) {
...
Connection.prototype.handleSocketError = function (err) {
  this.log('error', 'Socket error ' + err, 'r/w:', this.netClient.readable, this.netClient.writable);
...
Connection.prototype.invokePendingCallbacks = function (innerError) {
  ...
  if (handlers.length > 0) {
    this.log('verbose', 'Invoking ' + handlers.length + ' pending callbacks');
  }
...
Connection.prototype.authenticate = function(authenticator, token, callback) {
  ...
  var request = new writers.AuthResponseRequest(token);
...
Connection.prototype.changeKeyspace = function (keyspace, callback) {
  ...
  var self = this;
  this.sendStream(
    new writers.QueryWriter(query, null, null),
    null,
    function (err) {
      if (!err) {
...
Connection.prototype.getStreamId = function(callback) {
  ...
  if (!this.availableStreamIds) {
    ...
    //TODO: Replace queue with events when there is an available streamId
...
Connection.prototype.close = function disconnect (callback) {
  ...
  //TODO: Check inFlight() and timeout
  if(!callback) {
    callback = function () {};
  }
  if (!this.netClient) {
