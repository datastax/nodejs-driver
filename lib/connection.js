...
function Connection(endpoint, protocolVersion, options) {
  ...
  this.streamHandlers = {};
  this.continuousPagingStreamIds = new utils.HashSet();
  this.pendingWrites = [];
  this.preparing = {};
  /**
...
Connection.prototype.bindSocketListeners = function() {
  ...
  resultEmitter.on('lastContinuousPage', this.handleLastContinuousPage.bind(this));
  resultEmitter.on('byRowCompleted', this.handleByRowCompleted.bind(this));
...
Connection.prototype.getWriteCallback = function (request, options, callback) {
  ...
  return (function writeCallback (err) {
    ...
    self.setByRowOptions(options, request);
...
Connection.prototype.setByRowOptions = function (options, request) {
  if (!options || !options.rowCallback) {
    return;
  }
  if (!request.query) {
    // Only ExecuteRequest and QueryRequest allowed
    return;
  }
  this.parser.setOptions(request.streamId, { byRow: true });
  if (options.continuousPaging) {
    this.continuousPagingStreamIds.add(request.streamId);
    var wasRequested = options.cancelHandler;
    options.cancelHandler = this.getCancelHandler(request.streamId);
    options.resumeReadingHandler = this.getResumeReadingHandler();
    if (wasRequested) {
      // Execute after current IO events, hopefully after the writing of the actual request was made
      setImmediate(function delayedCancel() {
        options.cancelHandler(wasRequested);
      });
    }
  }
};
...
/**
 * Returns a function that can be used to cancel a request in this connection.
 * @param streamId
 * @return {Function}
 */
Connection.prototype.getCancelHandler = function (streamId) {
  var self = this;
  return (function connectionCancelHandler(callback) {
    self.sendStream(new requests.CancelRequest(streamId), null, function (err, response) {
      var handler = self.streamHandlers[streamId];
      if (!handler) {
        self.log('info', 'Continuous paging cancel response for frame #' + streamId + ' was obtained but streamId' +
          ' not found in the connection state');
        // It's possible that the cancel request was send while it was processing the last page and the handler is
        // already freed
        return;
      }
      if (err) {
      ...
      var row = response.rows && response.rows[0];
      var cancelled = row && row.get(0);
      if (cancelled) {
        // When the CANCEL response is obtained, it's guaranteed that no more frames with the same streamId will be read
        delete self.continuousPagingStreamIds.remove(streamId);
        self.log('verbose', 'Continuous paging cancelled for frame #' + streamId);
        self.parser.clearOptions(streamId);
        self.invokeCallback(handler, null, { });
        self.freeStreamId(streamId);
      }
      else {
        self.log('info', 'Continuous paging cancel response for frame #' + streamId + ' was obtained but was not' +
...
Connection.prototype.getResumeReadingHandler = function () {
  var self = this;
  return (function resumeReadingHandler(resume) {
    if (!resume && !self.isPaused) {
      self.isPaused = true;
      self.netClient.unpipe();
      // There's no point in issuing the idle timeout as the connection is not readable
      if (self.options.pooling.heartBeatInterval && self.idleTimeout) {
        clearTimeout(self.idleTimeout);
        self.idleTimeout = null;
      }
    }
    else if (resume && self.isPaused) {
      self.isPaused = false;
      self.netClient.pipe(self.protocol);
...
Connection.prototype.idleTimeoutHandler = function () {
  ...
  this.sendStream(new requests.QueryRequest(idleQuery), utils.emptyObject, function (err) {
    ...
    if (self.isPaused && (err instanceof errors.OperationTimedOutError)) {
      // It's possible that the idle timeout request timed out while this connection is paused
      // due to continuous paging back pressure mechanism: it's safe to ignore
      return;
    }
...
Connection.prototype.freeStreamId = function(streamId) {
  ...
  if (this.continuousPagingStreamIds.contains(streamId)) {
    // The parser finished parsing one of the frames but more are coming with the same streamId
    this.parser.setOptions(streamId, { byRow: true });
    return;
  }
...
Connection.prototype.handleLastContinuousPage = function (header) {
  this.log('verbose', 'Received last continuous page for frame #' + header.streamId);
  this.continuousPagingStreamIds.remove(header.streamId);
};

Connection.prototype.handleByRowCompleted = function (header, row, meta, flags) {
  var streamId = header.streamId;
  if (streamId < 0) {
    return this.log('error', 'Unexpected event received', header);
  }
  var handler = this.streamHandlers[streamId];
  if (!handler) {
    return this.log('error', 'The server replied with a wrong streamId for completed streaming frame #' + streamId);
  }
  if (this.continuousPagingStreamIds.contains(streamId)) {
    // Until the last continuous page, there is no need to invoke final callback
    return;
  }
  this.invokeCallback(handler, null, { rowLength: handler.rowIndex, meta: meta, flags: flags });
};
...
Connection.prototype.handleResult = function (header, err, result) {
  ...
  if (err) {
    // Remove the streamId from continuous paging as we don't expect / don't care of further pages
    this.continuousPagingStreamIds.remove(header.streamId);
  }
 ...
 */
Connection.prototype.handleRow = function (header, row, rowLength) {
  var streamId = header.streamId;
  if(streamId < 0) {
    return this.log('verbose', 'Event received', header);
  }
  var handler = this.streamHandlers[streamId];
  if (!handler) {
    return this.log('error', 'The server replied with a wrong streamId for streaming row #' + streamId);
  }
  if (handler.timeout) {
