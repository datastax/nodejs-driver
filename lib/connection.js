 ...
 * @param {ExecutionInfo|null} info
 * @param {function} callback Function to be called once the response has been received
 * @return {OperationState}
 */
Connection.prototype.sendStream = function (request, info, callback) {
  info = info || ExecutionInfo.empty();
  const operation = new OperationState(request, info.getRowCallback(), callback);
  const streamId = this._getStreamId();

  // Start the request timeout without waiting for the request to be written
  operation.setRequestTimeout(info, this.options.socketOptions.readTimeout, this.endpoint,
