...
Connection.prototype.handleData = function handleData(data) {
  ...
  if (headerReader.isCompleteFrame()) {
    ...
    this.emit('log', 'info', 'Received - start partial #' + headerReader.streamId);
  }
  else {
    //partial continue
    //First in first out
    ...
    if (totalLength < partial.header.bodyLength+8) {
      ...
      this.emit('log', 'info', 'Received partial continue #' + partial.header.streamId, totalLength, partial.header.bodyLength+8);
    }
