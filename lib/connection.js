    ...
    this.sendStream(request, null, function responseCallback(err, response) {
      if (err && self._checkingVersion) {
        ...
        if (invalidProtocol) {
          ...
          } else if (!types.protocolVersion.isDse(self.protocol.version) &&
                      !types.protocolVersion.isSupportedCassandraVersion(self.protocol.version)) {
            // Downshift if we get an unexpectedly large Cassandra version
            lowerVersion = types.protocolVersion.getLowerSupported(self.protocol.version);
          }
