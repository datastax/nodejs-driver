 ...
 */
Connection.prototype.executePrepared = function (queryId, params, consistency, options, callback) {
  if (typeof callback === 'undefined') {
    if (typeof options !== 'function') {
      throw new types.DriverError('Can not find callback, only the argument options is not required');
    }
    callback = options;
    options = null;
  }
  this.emit('log', 'info', 'executing prepared query: 0x' + queryId.toString('hex'));
  this.sendStream(new Writers.ExecuteWriter(queryId, params, consistency), callback, null, options);
}
