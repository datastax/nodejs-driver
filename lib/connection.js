...
Connection.prototype.handleData = function handleData(data) {
  ...
  if (headerReader.isCompleteFrame()) {
    //normal
    handleCompleteStream.call(this, data, headerReader);
  }
  else if (headerReader.isFrameResponseStart()) {
    //Start partial
    this.partialStreams.push({parts: [data], header: headerReader});
    this.emit('log', 'info', 'Start partial #' + headerReader.streamId);
  }
  else {
    //partial continue
    //First in first out
    ...
    if (totalLength === partial.header.bodyLength+8) {
      ...
      handleCompleteStream.call(this, Buffer.concat(partial.parts, totalLength), partial.header);
    }
    else {
      //just keep swimming
      this.partialStreams.unshift(partial);
      this.emit('log', 'info', 'Partial continue #' + partial.header.streamId, totalLength, partial.header.bodyLength+8);
    }
  }
  
  function handleCompleteStream(data, headerReader) {
    var streamId = headerReader.streamId;
    if(streamId >= 0) {
      this.emit('log', 'info', (this instanceof Connection));
