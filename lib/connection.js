...
Connection.prototype.createSocket = function() {
  ...
  self.netClient.on('readable', function (){
    ...
    if (totalLength > 0) {
      ...
      self.handleData(Buffer.concat(data, totalLength));
      //});
    }
  });
...
Connection.prototype.handleData = function handleData(data) {
  ...
  if (headerReader.isCompleteFrame()) {
    ...
    if (totalLength < partial.header.bodyLength+8) {
      ...
      var nextStart = partial.header.bodyLength+8-(totalLength-data.length);
      
      var nextHeader = ResponseHandlers.FrameHeaderReader.getFrameHeader(data, nextStart);
      if (nextHeader !== null && !nextHeader.isFrameResponseStart()) {
      ...
      var nextMessageStart = new Buffer(data.length-nextStart);
      data.copy(previousMessageEnd, 0, 0, nextStart);
      data.copy(nextMessageStart, 0, nextStart, data.length);
      
      //start next message partial
      this.partialStreams.push({parts: [nextMessageStart], header: nextHeader});
      ...
      this.handleFrame(Buffer.concat(partial.parts, totalLength), partial.header);
      if (nextHeader === null) {
        //There isn't enough data to read the full header
        this.emit('log', 'info', 'Starting partial: there is not enough data to read the header');
      }
      else {
        this.emit('log', 'info', 'Start partial (as next message) #' + nextHeader.streamId);
      }
      partial = null;
