...
Connection.prototype.executePrepared = function () {
  ...
  if (args.options && args.options.streamRows && !args.rowCallback) {
    args.rowCallback = args.callback;
    args.callback = null;
  }
  this.emit('log', 'trace', args.callback, args.rowCallback);
  this.sendStream(
    new writers.ExecuteWriter(args.query, args.params, args.consistency),
    utils.extend({}, args.options, {rowCallback: args.rowCallback}),
    args.callback);
};
 ...
 * @param {function} [callback]
 */
Connection.prototype.sendStream = function sendStream (frameWriter, options, callback) {
  var self = this;
