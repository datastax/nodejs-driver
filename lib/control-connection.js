...
class ControlConnection extends events.EventEmitter {
  ...
  async _refreshControlConnection(hostIterator) {
    ...
    try { this.connection = this._borrowAConnection(hostIterator); }
      ...
      this.log("info", "ControlConnection could not reconnect using existing connections", err);
      if (this.options.cloud) {

        this.log("info", "Refreshing host IDs for cloud nodes from metadata service");
        // Cloud metadata service can fail if all nodes are shutting down and new nodes are being
        // setup.  In this case we want the refresh process to complete normally so that the connection
        // created above is properly initialized.
        try { await cloud.init(this.options); }
        catch(err) {

          this.log('error',"Failed to retrieve host IDs from metadata service", err);
        }
