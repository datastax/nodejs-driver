...
class ControlConnection extends events.EventEmitter {
   ...
   */
  async refreshHostsAsync(initializing, isReconnecting) {
  ...
  async refreshAsync(reuseQueryPlan) {
    ...
    try {
      ...
      if (!this.isShuttingDown) {
        ...
        setTimeout(() => this.refreshAsync(), delay);
    ...
    try {
      ...
      await this.refreshHostsAsync(initializing, true);
  ...
  scheduleRefreshHosts(callback) {
    this.debouncer.eventReceived({
      ...
      handler: cb => this.refreshHostsAsync(false, false).then(cb).catch(cb),
      all: true,
      callback
    }, false);
  }
