...
ControlConnection.prototype.nodeStatusChangeHandler = function (event) {
  var self = this;
  this.addressTranslator.translate(event.inet.address, this.options.protocolOptions.port, function (endPoint) {
...
ControlConnection.prototype.nodeSchemaChangeHandler = function (event) {
  ...
  if (event.schemaChangeType !== schemaChangeTypes.created) {
    if (event.table) {
      ...
      delete this.metadata.keyspaces[event.keyspace].tables[event.table];
      return;
    }
    if (event.udt) {
      //Is a user defined type change, clean the internal cache
      delete this.metadata.keyspaces[event.keyspace].udts[event.udt];
      return;
    }
    if (event.functionName) {
      //Is a function change, clean the internal cache
      delete this.metadata.keyspaces[event.keyspace].functions[event.functionName];
      return;
    }
    if (event.aggregate) {
      //Is a aggregate change, clean the internal cache
      delete this.metadata.keyspaces[event.keyspace].aggregates[event.aggregate];
      return;
    }
