...
class ControlConnection extends events.EventEmitter {
   ...
   */
  async _borrowAConnection(hostIterator) {
    let connection = null;
    ...
    while (!connection) {
      ...
      try {
        connection = await this._borrowHostConnection(host);
      } catch (err) {
  ...
  async _refresh(hostIterator) {
    ...
    try {
      ...
      await this._borrowAConnection(hostIterator);
  ...
  _nodeTopologyChangeHandler(event) {
    ...
    this._topologyChangeTimeout = setTimeout(() => this._scheduleRefreshHosts(), newNodeDelay);
  }
  ...
  handleSchemaChange(event, processNow, callback) {
    ...
    if (event.isKeyspace) {
      if (event.schemaChangeType === schemaChangeTypes.dropped) {
        ...
        return this._scheduleObjectRefresh(handler, event.keyspace, null, processNow, callback);
      }
      return this._scheduleKeyspaceRefresh(event.keyspace, processNow, callback);
    }
    ...
    if (handler) {
      ...
      this._scheduleObjectRefresh(handler, event.keyspace, cqlObject, processNow, callback);
    }
  }
   ...
   * @param {Function} [callback]
   */
  _scheduleObjectRefresh(handler, keyspaceName, cqlObject, processNow, callback) {
   ...
   * @param {Function} [callback]
   */
  _scheduleKeyspaceRefresh(keyspaceName, processNow, callback) {
  ...
  _scheduleRefreshHosts() {
    this._debouncer.eventReceived({
      //TODO: Remove callback or do async
      handler: cb => this._refreshHosts(false, false).then(cb).catch(cb),
      all: true
    }, false);
  }
  ...
  query(cqlQuery, waitReconnect, callback) {
    ...
    if (!this.connection && waitReconnect) {
      ...
      return this._waitForReconnection(function waitCallback(err) {
        if (err) {
  ...
  async reset() {
    ...
    await Promise.all(currentHosts.map(h => h.shutdownAsync(false).catch(() => {})));
