...
ControlConnection.prototype.nodeStatusChangeHandler = function (event) {
  var endPoint = this.options.policies.addressResolution.translate(event.inet.address, this.options.protocolOptions.port);
  var host = this.hosts.get(endPoint);
  if (!host) {
    this.log('warning', 'Received status change event but host was not found: ' + event.inet.address);
    return;
  }
  if (event.up) {
...
ControlConnection.prototype.setPeersInfo = function (newNodesUp, result) {
  ...
  result.rows.forEach(function (row) {
    var endPoint = self.getAddressForPeerHost(row, port);
    if (!endPoint) {
      return;
    }
    peers[endPoint] = true;
 ...
 * @returns {String} The string representation of the host endpoint, containing the ip address and port
 */
ControlConnection.prototype.getAddressForPeerHost = function (row, defaultPort) {
  ...
  return this.options.policies.addressResolution.translate(address.toString(), defaultPort);
};

ControlConnection.prototype.getLocalSchemaVersion = function (callback) {
  this.query(selectSchemaVersionLocal, function (err, result) {
    var version;
