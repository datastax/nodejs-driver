...
ControlConnection.prototype.init = function (callback) {
  ...
  if (!this.options.sni) {
    ...
    if (net.isIP(nameOrIp)) {
      this.options.sni.ip = nameOrIp;
      return contactPointsResolutionCb();
    }

    dns.resolve4(nameOrIp, (err, arr) => {
      if (!err && !arr || arr.length === 0) {
        err = new Error(`${nameOrIp} could not be resolved`);
      }

      // Use the first resolved IP
      this.options.sni.ip = arr && arr[0];

      contactPointsResolutionCb(err);
    });
  }
};
