...
class ControlConnection extends events.EventEmitter {
   ...
   */
  async borrowAConnection(hostIterator) {
    let host;
    let connection = null;
  ...
  async refresh(hostIterator) {
    ...
    try {
      ...
      await this.borrowAConnection(hostIterator);
  ...
  nodeTopologyChangeHandler(event) {
    ...
    this._topologyChangeTimeout = setTimeout(() => this.scheduleRefreshHosts(), newNodeDelay);
  }
  ...
  scheduleObjectRefresh(handler, keyspaceName, cqlObject, processNow, callback) {
    this._debouncer.eventReceived({ handler, keyspace: keyspaceName, cqlObject: cqlObject, callback }, processNow);
  }
  ...
  query(cqlQuery, waitReconnect, callback) {
    ...
    function queryOnConnection() {
      if (!self.connection || self._isShuttingDown) {
