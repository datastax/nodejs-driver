...
ControlConnection.prototype.init = function (callback) {
  ...
  tasks.push(this.initOnConnection.bind(this));
...
ControlConnection.prototype.getConnection = function (callback) {
  ...
  if (!this.host) {
    ...
    handler.getNextConnection(function (err, c, host) {
      if (err) {
 ...
 */
ControlConnection.prototype.initOnConnection = function (callback) {
  ...
  async.series([
    ...
    function initDone(err) {
      if (err) {
        ...
        self.log('info', 'ControlConnection connected');
      }
      callback(err);
    });
};
...
ControlConnection.prototype.hostDownHandler = function () {
  ...
  async.series([
      ...
      this.initOnConnection.bind(this)
    ],
    function (err) {
      if (err) {
...
ControlConnection.prototype.log = function (type, info, furtherInfo) {
  this.emit('log', type, info, this.constructor.name, furtherInfo || '');
};
