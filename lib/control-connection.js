...
class ControlConnection extends events.EventEmitter {
  ...
  async borrowAConnection() {
    //TODO: Maybe split between initialization and lbp iteration
    let host;
    let connection = null;
  ...
  async refresh(reuseQueryPlan) {
    ...
    try {
      ...
      if (initializing) {
        ...
        const hosts = utils.shuffleArray(Array.from(this._contactPoints));
        this.hostIterator = hosts[Symbol.iterator]();
        this.triedHosts = {};
      ...
      if (initializing) {
        // This method will only throw when initializing
        // Otherwise it will schedule reconnection attempts
        throw err;
      }
    ...
    try {
      ...
      await this.refreshHosts(initializing, true);
