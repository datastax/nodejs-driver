...
class ControlConnection extends events.EventEmitter {
  ...
  async parseEachContactPoint(name) {
    ...
    const addresses = await this.resolveAll(addressOrName);
  ...
  async init() {
    ...
    if (!this.options.sni) {
      ...
      await Promise.all(this.options.contactPoints.map(name => this.parseEachContactPoint(name)));
    } else {
   ...
   */
  async borrowAConnection() {
    let host;
    let connection = null;

    while (!connection) {
      const item = this.hostIterator.next();
      ...
      const distance = this.profileManager.getDistance(host);
      if (!host.isUp() || distance === types.distance.ignored) {
        continue;
      }

      try {
        connection = await this.borrowHostConnection(host);
      } catch (err) {
   ...
   */
  async borrowFirstConnection(contactPoints) {
    let connection = null;

    for (const cp of contactPoints) {
      try {
        connection = await this.createConnection(cp);

        // We have an open connection, exit the loop
        break;
      } catch (err) {
        this.triedHosts[cp] = err;
      }
    }

    if (!connection) {
      const err = new errors.NoHostAvailableError(this.triedHosts);
  ...
  borrowHostConnection(host) {
    // Borrow any open connection, regardless of the keyspace
    return host.borrowConnectionAsync(null, null);
  }
   ...
   */
  async createConnection(contactPoint) {
   ...
   */
  async refreshHosts(initializing, isReconnecting) {
    ...
    this.setLocalInfo(initializing, isReconnecting, c, rsLocal);
   ...
   * <p>If it fails obtaining a connection and there aren't any more hosts, it schedules reconnection</p>
   * <p>If it fails obtaining the metadata:</p>
   * <ul>
   *   <li>It marks connection and/or host unusable</li>
   *   <li>Retry using the same iterator from query plan / host list</li>
   * </ul>
   * @param {Boolean} [reuseQueryPlan]
   */
  async refresh(reuseQueryPlan) {
    ...
    try {
      ...
      await this.refreshHosts(false, true);

      await this.registerToConnectionEvents();
    } catch (err) {
  ...
  async initializeConnection() {
    ...
    const contactPoints = utils.shuffleArray(Array.from(this._contactPoints));

    while (true) {
      await this.borrowFirstConnection(contactPoints);
      ...
      try {
        await this.getSupportedOptions();
        await this.refreshHosts(true, true);
        await this.registerToConnectionEvents();
  ...
  async registerToConnectionEvents() {
  ...
  nodeTopologyChangeHandler(event) {
    ...
    this.topologyChangeTimeout = setTimeout(() => this.scheduleRefreshHosts(), newNodeDelay);
  }
  ...
  scheduleKeyspaceRefresh(keyspaceName, processNow, callback) {
    this.debouncer.eventReceived({
      //TODO: Remove callback or do async
  ...
  scheduleRefreshHosts(callback) {
    this.debouncer.eventReceived({
      //TODO: Remove callback or do async
      handler: cb => this.refreshHosts(false, false).then(cb).catch(cb),
      all: true,
      callback
    }, false);
  }
  ...
  getAddressForPeerHost(row, defaultPort) {
    return new Promise(resolve => {
      ...
      this.addressTranslator.translate(address.toString(), defaultPort, resolve);
    });
  }
