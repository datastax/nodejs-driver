...
Client.prototype._setGraphOptions = function (options) {
  ...
  this._setPayloadKey(options, 'graph-read-consistency', clientOptions.getConsistencyName(options.graphReadConsistency));
  this._setPayloadKey(options, 'graph-write-consistency', clientOptions.getConsistencyName(options.graphWriteConsistency));
  ...
  if (!options) {
    ...
    if (readConsistency !== undefined) {
      options.customPayload['graph-read-consistency'] =
        utf8Buffer(clientOptions.getConsistencyName(readConsistency));
    }
    ...
    if (writeConsistency !== undefined) {
      options.customPayload['graph-write-consistency'] =
        utf8Buffer(clientOptions.getConsistencyName(writeConsistency));
    }
