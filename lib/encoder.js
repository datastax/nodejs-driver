...
function defineInstanceMembers() {
   ...
   * 
   * @param {Buffer} bytes 
   * @param {ColumnInfo | string | [ColumnInfo, number]} typeName
   * @returns 
   */
  this.decodeCustom = function (bytes, typeName) {
    ...
    if (Array.isArray(typeName) && typeof typeName[1] === 'number') {
      return this.decodeVector(bytes, {code: dataTypes.vector, info: typeName, customTypeName: 'vector'});
    }
   ...
   * 
   * @param {any} value 
   * @param {string | ColumnInfo | [ColumnInfo, number]} customTypeName when it's a vector, it will be [subtype, dimension]
   * @returns 
   */
  this.encodeCustom = function (value, customTypeName) {
    ...
    // Unless we say as long as customTypeName is an array where the second item is a number, it's a vector?
    if (Array.isArray(customTypeName) && typeof customTypeName[1] === 'number') {
      return this.encodeVector(value, { code: dataTypes.vector, info: customTypeName , customTypeName: 'vector'});
    }
...
Encoder.prototype.encode = function (value, typeInfo) {
  ...
  return encoder.call(this, value, 'info' in type ? type.info : null);
};
