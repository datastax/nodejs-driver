...
function defineInstanceMembers() {
  ...
  const customDecoders = {
    ...
    [customTypeNames.vector]: decodeVector
  };
  ...
  const customEncoders = {
    ...
    [customTypeNames.vector]: encodeVector
  };
  ...
  this.parseFqTypeName = function (typeName, startIndex, length) {
    ...
    if (typeName.indexOf(customTypeNames.vector, startIndex) === startIndex) {
      ...
      startIndex += complexTypeNames.vector.length + 1;
      length -= complexTypeNames.vector.length + 2;
      params = parseParams(typeName, startIndex, length);
      if (params.length !== 2) {
        throw new TypeError('Not a valid type ' + typeName);
      }
      dataType.code = dataTypes.vector;
      dataType.info = [this.parseFqTypeName(params[0]), parseInt(params[1])];
      return dataType;
    }
...
Encoder.prototype.encode = function (value, typeInfo) {
  ...
  if (typeInfo) {
    ...
    console.log("Found type " + util.inspect(type));
    if (!type) {
...
Encoder.guessDataType = function (value) {
  ...
  else if (value instanceof types.CqlVector) {
    code = dataTypes.custom;
...
function decodeVector(buffer, params) {
 ...
 * @param {Array} params
 */
function encodeVector(value, params) {
  if (!(value instanceof types.CqlVector)) {
    throw new TypeError('Expected CqlVector, obtained ' + util.inspect(value));
  }
  if (value.length === 0) {
    throw new TypeError("Cannot encode empty CqlVector");
  }
  let subtype = typeof(value[0]);
  if (params) {
