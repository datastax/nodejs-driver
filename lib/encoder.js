...
// Avoid using BigInt literals like 32n as we must be able to compile with older engines
...
function defineInstanceMembers() {
  ...
  this._decodeBigIntAsLong = function (bytes) {
    return Long.fromBuffer(bytes);
  };

  this._decodeBigIntAsBigInt = function (bytes) {
    return BigInt.asIntN(64, BigInt(bytes.readInt32BE(4)) | (BigInt(bytes.readInt32BE(0)) << bigInt32));
  };

  this.decodeLong = this.encodingOptions.useBigIntAsLong
    ? this._decodeBigIntAsBigInt
    : this._decodeBigIntAsLong;
  ...
  this.decodeTimestamp = function(bytes) {
    return new Date(this._decodeBigIntAsLong(bytes).toNumber());
  };
