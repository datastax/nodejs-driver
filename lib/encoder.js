...
function defineInstanceMembers() {
  ...
  this.decodeCustom = function (bytes, typeName) {
    ...
    if (typeName.indexOf(customTypeNames.vector) >= 0) {
      return this.decodeVector(bytes, this.parseVectorTypeArgs(typeName))
    }
    else {
  ...
  this.encodeCustom = function (value, customTypeName) {
    ...
    if (customTypeName.indexOf(customTypeNames.vector) >= 0) {
      return this.encodeVector(value, this.parseVectorTypeArgs(customTypeName))
    }
    else {
   ...
   * @internal
   */
  this.parseVectorTypeArgs = function(typeName) {

    const argsStartIndex = customTypeNames.vector.length + 1;
    const argsLength = customTypeNames.vector.length + 2;
    const params = parseParams(typeName, argsStartIndex, argsLength);
    if (params.length === 1 && params[0] === "") {
      return []
    }
    if (params.length === 2) {
      return [this.parseFqTypeName(params[0]), parseInt(params[1])];
    }
  ...
  this.parseFqTypeName = function (typeName, startIndex, length) {
    ...
    if (typeName.indexOf(customTypeNames.vector, startIndex) === startIndex) {
      ...
      dataType.info = parseVectorTypeArgs(typeName);
      return dataType;
    }
