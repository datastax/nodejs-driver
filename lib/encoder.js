...
function defineInstanceMembers() {
   ...
   * @ignore
   */
  this.setRoutingKeyFromUser = function (params, info, keys) {
    let totalLength = 0;
    const userRoutingKey = info.getRoutingKey();
    if (util.isArray(userRoutingKey)) {
      if (userRoutingKey.length === 1) {
        info.setRoutingKey(userRoutingKey[0]);
        return;
      }
      ...
      for (let i = 0; i < userRoutingKey.length; i++) {
        ...
        if (!item) {
          ...
          info.setRoutingKey(null);
          return;
        }
        totalLength += item.length + 3;
      }

      info.setRoutingKey(concatRoutingKey(userRoutingKey, totalLength));
      return;
    }
    ...
    let routingIndexes = info.getRoutingIndexes();
    if (info.getRoutingNames()) {
      routingIndexes = info.getRoutingNames().map(k => keys[k]);
    }
    if (!routingIndexes) {
      return;
    }
    ...
    const parts = [];
    const hints = info.getHints() || utils.emptyArray;
    ...
    info.setRoutingKey(concatRoutingKey(parts, totalLength));
  };
   ...
   * @ignore
   */
  this.setRoutingKeyFromMeta = function (meta, params, info) {
    const routingIndexes = info.getRoutingIndexes();
    if (!routingIndexes) {
      return;
    }
