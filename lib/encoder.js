...
const DateRange = require('./datastax/search').DateRange;
...
function defineInstanceMembers() {
  ...
  this.parseTypeNameAsync = async function (keyspace, typeName, startIndex, length, udtResolver) {
    startIndex = startIndex || 0;
    if (!length) {
      length = typeName.length;
    }
    ...
    if (typeName.indexOf(cqlNames.list, startIndex) === startIndex) {
      ...
      dataType.code = dataTypes.list;
      dataType.info = await this.parseTypeNameAsync(keyspace, innerTypes[0], 0, null, udtResolver);
      return dataType;
    }
    ...
    if (typeName.indexOf(cqlNames.map, startIndex) === startIndex) {
      ...
      dataType.code = dataTypes.map;
      return this._parseChildTypesAsync(keyspace, dataType, innerTypes, udtResolver);
    }
  ...
  // TODO: Remove couterpart
  this._parseChildTypesAsync = async function (keyspace, dataType, typeNames, udtResolver) {
    dataType.info = await Promise.all(typeNames.map(name =>
      this.parseTypeNameAsync(keyspace, name.trim(), 0, null, udtResolver)));
  };
