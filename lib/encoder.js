...
function defineInstanceMembers() {
  ...
  this.decodeCustom = function (bytes, typeName) {
    ...
    if (typeName.indexOf(customTypeNames.vector) >= 0) {
      return this.decodeVector(bytes, this.parseVectorTypeArgs(typeName, customTypeNames.vector))
    }
    else {
  ...
  this.encodeCustom = function (value, customTypeName) {
    ...
    // Special handling for vector custom types (since they have args)
    if (customTypeName.indexOf(customTypeNames.vector) >= 0) {
      return this.encodeVector(value, this.parseVectorTypeArgs(customTypeName, customTypeNames.vector))
    }
    else {
  ...
  this.encodeVector = function(value, params) {
    ...
    function evalParams() {
      ...
      if (params.length === 2) {
        ...
        // If we didn't get full params (i.e. if we're working with simple statements) then try to infer subtype
   ...
   * @param {Function} subtypeFn Function used to resolve subtype type; varies depending on type naming convention
   * @returns {Array}
   * @internal
   */
  this.parseVectorTypeArgs = function(typeName, stringToExclude, subtypeFn) {

    // Most cases involve the FQ case so default to that
    const resolveFn = subtypeFn || this.parseFqTypeName;

    const argsStartIndex = stringToExclude.length + 1;
    const argsLength = stringToExclude.length + 2;
    const params = parseParams(typeName, argsStartIndex, argsLength);
    if (params.length === 1 && params[0] === "") {
      return []
    }
    if (params.length === 2) {
      return [resolveFn(params[0]), parseInt(params[1])];
    }
  ...
  this.parseTypeName = async function (keyspace, typeName, startIndex, length, udtResolver) {
    ...
    if (typeName.indexOf(cqlNames.vector, startIndex) === startIndex) {
      ...
      const resolveFn = this.parseTypeName
      dataType.code = dataTypes.custom;
      dataType.info = this.parseVectorTypeArgs(typeName, cqlNames.vector, function (typeArg) { return resolveFn(keyspace, typeArg) });
      return dataType;
    }
  ...
  this.parseFqTypeName = function (typeName, startIndex, length) {
    ...
    if (typeName.indexOf(customTypeNames.vector, startIndex) === startIndex) {
      // It's a vector, so record the subtype and dimension.
      dataType.code = dataTypes.custom;
      dataType.info = parseVectorTypeArgs(typeName, customTypeNames.vector);
      return dataType;
    }
