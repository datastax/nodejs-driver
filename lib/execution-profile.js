...
function ProfileManager(options) {
  ...
  this.profilesArray = Object.keys(this.profiles).map(function (name) {
    // set required properties
    var p = this.profiles[name];
...
ProfileManager.prototype.init = function (client, hosts, callback) {
  utils.eachSeries(this.profilesArray, function (profile, next) {
...
ProfileManager.prototype.getDistance = function (host) {
  ...
  for (var i = 0; i < this.profilesArray.length; i++) {
    var d = this.profilesArray[i].loadBalancing.getDistance(host);
    if (d < distance) {
      distance = d;
...
ProfileManager.prototype.getProfile = function (name) {
  ...
  return this.profiles[name || 'default'];
};
 ...
 * @param {ClientOptions} options
 */
ProfileManager.prototype.setDefault = function (options) {
  this._defaultProfile = this.profiles['default'];
  if (!this._defaultProfile) {
    this._defaultProfile = this.profiles['default'] = new ExecutionProfile();
  }
