...
class HostConnectionPool extends events.EventEmitter {
  ...
  async createAndBorrowConnectionAsync(keyspace) {
    await this.createAsync(false);

    return await this.borrowConnectionAsync(keyspace, null);
  }
  ...
  //TODO: Remove counterpart
  async borrowConnectionAsync(keyspace, previousConnection) {
    if (this.connections.length === 0) {
    ...
    if (keyspace && keyspace !== c.keyspace) {
      await c.changeKeyspaceAsync(keyspace);
    }
   ...
   */
  //TODO: Rename and refine usage / behaviour
  borrowSync(keyspace) {
    if (this.connections.length === 0) {
      return null;
    }
    ...
    const maxRequests = this.options.pooling.maxRequestsPerConnection;
    const c = HostConnectionPool.minInFlight(this.connections, maxRequests, null);

    if (c.getInFlight() >= maxRequests || (keyspace && keyspace !== c.keyspace)) {
      return null;
    }
  ...
  //TODO: Remove counterpart
  async createAsync(warmup) {
    if (this.isClosing()) {
      throw new Error('Pool is being closed when calling create');
    }
    ...
    if (this._creating) {
      // wait for the pool to finish creating
      return await promiseUtils.fromEvent(this, 'creation');
    }

    this._creating = true;
    const connectionsToCreate = warmup ? this.coreConnectionsLength : 1;

    while (this.connections.length < connectionsToCreate) {
      try {
        await this._attemptNewConnectionAsync();
      } catch (err) {
        if (this.isClosing()) {
          this.log('info', 'Connection pool created but it was being closed');
          this._closeAllConnections();
        } else {
          // there was an error and no connections could be successfully opened
          this.log('warning', `Connection pool to host ${this._address} could not be created`, err);
        }

        this._creating = false;
        this.emit('creation', err);
    ...
    this._creating = false;
    this.log('info', `Connection pool to host ${this._address} created with ${this.connections.length} connection(s)`);
    this.emit('creation');
    this.increaseSize();
  }
  ...
  //TODO: Remove counterpart
  async _attemptNewConnectionAsync() {
    if (this._opening) {
      // wait for the event to fire
  ...
  scheduleNewConnectionAttempt(delay) {
    ...
    this._newConnectionTimeout = setTimeout(function newConnectionTimeoutExpired() {
      ...
      if (delay > 0 && self.options.sni) {
        ...
        self.options.sni.addressResolver.refresh().then(() => self._attemptNewConnection(utils.noop));
        return;
      }
