...
//TODO: Replace counterpart
Host.prototype.shutdownAsync = util.promisify(Host.prototype.shutdown);
...
//TODO: Remove counterpart and mark previousConnection as optional
Host.prototype.borrowConnectionAsync = function (keyspace, previousConnection) {
  if (previousConnection) {
    // Obtain one of the existing connections
    return this.pool.borrowConnectionAsync(keyspace, previousConnection);
  }

  return this.pool.createAndBorrowConnectionAsync(keyspace);
};

//TODO: Use one or the other
Host.prototype.borrowSync = function(keyspace) {
  return this.pool.borrowSync(keyspace);
};
 ...
 * @ignore
 */
Host.prototype.warmupPool = function () {
  return this.pool.createAsync(true);
};
 ...
 * @private
 */
Host.prototype._onNewConnectionOpen = async function (err) {
  if (err) {
    this._checkPoolState();
    return;
  }
  ...
  if (!this.isUp() && this.options.rePrepareOnUp) {
    ...
    try {
      ...
      this.log('warning', util.format('Failed re-preparing on host %s: %s', this.address, err), err);
    }
  }
 ...
 * @returns {Array}
 */
Host.prototype.getDseVersion = function () {
  if (!this.dseVersion) {
    return utils.emptyArray;
  }
