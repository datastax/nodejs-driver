...
class InsightsClient {
  ...
  _sendStartupEvent() {
    return this._getStartupMessage()
      .then(message => new Promise((resolve, reject) => {
        ...
        this._client.controlConnection.query(request, false, err => {
          if (!err) {
            resolve();
          } else {
            reject(err);
  ...
  _dseSupportsInsights() {
    ...
    return this._client.hosts.values().reduce((acc, host) => {
      ...
      const version = new VersionNumber(versionArr[0], versionArr[1], versionArr[2]);
  ...
  /**
   * @private
   * @returns {Promise} A Promise that will never be rejected
   */
