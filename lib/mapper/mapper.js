...
class Mapper {
  ...
  batch(items, executionOptions) {
    ...
    return Promise
      .all(items.map(item => item.pushQueries(queries).then(isIdempotent = isIdempotent && item.isIdempotent)))
      .then(() => this.client.batch(queries, DocInfoAdapter.adaptBatchOptions(executionOptions, isIdempotent)))
      .then(rs => {
