...
class MappingHandler {
  ...
  getSelectExecutor(doc, docInfo) {
    ...
    const fieldColumns = DocInfoAdapter.adaptFields(docInfo, this._info);
  ...
  getInsertExecutor(doc, docInfo) {
    ...
    return ObjectSelector.getForInsert(this._client, this._info, columnKeys, fieldColumns)
      .then(tableNames => {

        let columns = columnKeys;

        if (fieldColumns.length > 0) {
          // Only INSERT the fields provided
        ...
        const queries = tableNames.map(tableName => ({
          query: QueryGenerator.getInsert(tableName, columns, ifNotExists, ttl),
          paramsGetter: QueryGenerator.insertParamsGetter(docKeys, docInfo)
        }));
