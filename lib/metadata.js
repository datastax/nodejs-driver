...
/**
 * Gets the definition of a CQL function.
 * <p>
...
Metadata.prototype.getFunctions = function (keyspaceName, name, callback) {
  this._getFunctions(keyspaceName, name, function (err, functionsMap) {
    if (err || !functionsMap) {
...
/**
 * Gets a map of cql function definitions based on signature.
 * @private
 */
Metadata.prototype._getFunctions = function (keyspaceName, name, callback) {
  var keyspace = this.keyspaces[keyspaceName];
  if (!keyspace) {
    return callback(null, null);
  }
  ...
  //cache it
  //by name
  //within name, by signature ()
  var functionsInfo = keyspace.functions[name];
  if (!functionsInfo) {
    keyspace.functions[name] = functionsInfo = new events.EventEmitter();
    functionsInfo.setMaxListeners(0);
  }
  if (functionsInfo.values) {
    return callback(null, functionsInfo);
  }
  ...
  //it is not cached, try to query for it
  var self = this;
  var query = util.format(_selectFunctions, keyspace.name, name);
  self.controlConnection.query(query, function (err, response) {
    ...
    try {
      response.rows.forEach(function (row) {
        var func = new SchemaFunction(encoder, row);
...
TableMetadata.prototype.build = function (tableRow, columnRows) {
  ...
  if (tableRow['key_aliases'] && this.partitionKeys.length === 0) {
  ...
  if (tableRow['column_aliases'] && this.clusteringKeys.length === 0) {
  ...
  if (!this.isCompact) {
    ...
    this.isCompact = !!(
      ...
      (types && this.clusteringKeys.length !== (types.length - 1)));
  }
