...
function TableMetadata(name, encoder) {
  ...
  this.columns = [];
  /**
   * Array describing the columns that form the partition keys.
   * @type {Array}
   */
  this.partitionKeys = [];
  /**
...
TableMetadata.prototype.build = function (tableRow, columnRows) {
  ...
  if (!this.isCompact) {
    ...
    this.isCompact =
      ...
      tableRow['comparator'].indexOf('org.apache.cassandra.db.marshal.CompositeType') !== 0;
  }
...
function adaptKeyTypes(typesString, encoder) {
  return typesString
    ...
    .split(',')
    .map(function (x) {
      return encoder.parseTypeName(x);
    });
}
