...
Metadata.prototype.buildTokens = function (hosts) {
  ...
  hostArray.forEach((h) => {
    ...
    h.tokens.forEach((tokenString) => {
      ...
      primaryReplicas[stringify(token._value)] = h;
    });
  ...
  for (let i = 0; i < allSorted.length; i++) {
    this.ringTokensAsStrings[i] = stringify(allSorted[i]._value);
  }
...
Metadata.prototype.getTokenRangesForHost = function (keyspaceName, host) {
  ...
  this.tokenRanges.forEach((tokenRange) => {
    const replicasForToken = replicas[this.tokenizer.stringify(tokenRange.end._value)];
