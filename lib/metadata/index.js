...
class Metadata {
  ...
  async _getTrace(traceId, consistency) {
    ...
    while (!trace && (attempts++ < _traceMaxAttemps)) {
      const sessionResponse = await this.controlConnection.queryAsync(sessionRequest);
      ...
      const eventsResponse = await this.controlConnection.queryAsync(eventsRequest);
