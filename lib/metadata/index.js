 ...
 * </p>
 * @param {String} keyspaceName Name of the keyspace
 * @param {String} tableName Name of the materialized view base table
 ...
 */
Metadata.prototype.getMaterializedView = function (keyspaceName, tableName, name, callback) {
  var table;
  var view = null;
  var self = this;
  async.series([
    function getTableMetadata(next) {
      self.getTable(keyspaceName, tableName, function (err, t) {
        table = t;
        next(err);
      });
    },
    function getViewMetadata(next) {
      if (!table) {
        return next();
      }
      self._schemaParser.getMaterializedView(self.controlConnection, keyspaceName, table, name, function (err, v) {
        view = v;
        next(err);
      });
    }
