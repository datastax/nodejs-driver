...
Metadata.prototype.getReplicas = function (keyspaceName, tokenBuffer) {
  ...
  console.log('Hashed', tokenBuffer, 'as', token);
...
Metadata.prototype.getTokenRangesForHost = function (keyspaceName, host) {
  ...
  Object.keys(replicas).forEach((tokenStr) => {
    const replicasForToken = replicas[tokenStr];
    if(replicasForToken.includes(host)) {
      // identify ranges that start with this token.
      const token = this.tokenizer.parse(tokenStr);
