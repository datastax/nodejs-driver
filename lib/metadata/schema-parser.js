...
function getTokenToReplicaNetworkMapper(replicationFactors) {
  ...
  return (function tokenNetworkStrategy(tokenizer, ring, primaryReplicas, datacenters) {
    ...
    //Pre-compute string versions as it's expensive with potentially high iteration counts on the nested loops
    var ringAsStrings = [];
    for (var k = 0; k < ring.length; k++) {
      ringAsStrings.push(tokenizer.stringify(ring[k]));
    }

    for (var i = 0; i < ring.length; i++) {
      var key = ringAsStrings[i];
      var tokenReplicas = [];
      var replicasByDc = {};
      var racksPlaced = {};
      var skippedHosts = [];
      for (var j = 0; j < ring.length; j++) {
        ...
        var h = primaryReplicas[ringAsStrings[nextReplicaIndex]];
        var dc = h.datacenter;
