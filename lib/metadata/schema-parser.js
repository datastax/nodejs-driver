 ...
 */
SchemaParser.prototype.getKeyspace = function (name) {
};
 ...
 */
SchemaParser.prototype.getKeyspaces = function (waitReconnect) {
};
 ...
 */
SchemaParser.prototype.getTable = async function (keyspaceName, name, cache, virtual) {
...
SchemaParser.prototype._getFirstRow = async function (query) {
  const rows = await this._getRows(query);
  return rows[0];
};

SchemaParser.prototype._getRows = async function (query) {
  const response = await this.cc.queryAsync(query);
 ...
 */
SchemaParser.prototype.getUdt = async function (keyspaceName, name, cache) {
 ...
 * @abstract
 */
SchemaParser.prototype._parseUdt = function (udtInfo, row) {
};
 ...
 * @throws {Error}
 */
SchemaParser.prototype._parseTableOrView = async function (tableInfo, tableRow, columnRows, indexRows, virtual) {
};
 ...
 */
SchemaParser.prototype.getMaterializedView = function (keyspaceName, name, cache) {
 ...
 * @returns {Promise<Map>}
 */
SchemaParser.prototype.getFunctions = async function (keyspaceName, name, aggregate, cache) {
  /** @type {String} */
 ...
 * @returns {Promise}
 */
SchemaParser.prototype._parseAggregate = function (row) {
 ...
 * @returns {Promise}
 */
SchemaParser.prototype._parseFunction = function (row) {
};
...
/** @override */
SchemaParserV1.prototype.getKeyspaces = async function (waitReconnect) {
  const keyspaces = {};
  const result = await this.cc.queryAsync(_selectAllKeyspacesV1, waitReconnect);
...
/** @override */
SchemaParserV1.prototype.getKeyspace = async function (name) {
...
SchemaParserV1.prototype._parseTableOrView = async function (tableInfo, tableRow, columnRows, indexRows, virtual) {
...
/** @override */
SchemaParserV1.prototype.getMaterializedView = function (keyspaceName, name, cache) {
  return Promise.reject(
    new errors.NotSupportedError('Materialized views are not supported on Cassandra versions below 3.0'));
};

// eslint-disable-next-line require-await
SchemaParserV1.prototype._parseAggregate = async function (row) {
  const encoder = this.cc.getEncoder();
...
// eslint-disable-next-line require-await
SchemaParserV1.prototype._parseFunction = async function (row) {
  const encoder = this.cc.getEncoder();
...
// eslint-disable-next-line require-await
SchemaParserV1.prototype._parseUdt = async function (udtInfo, row) {
  const encoder = this.cc.getEncoder();
...
/** @override */
SchemaParserV2.prototype.getKeyspaces = async function (waitReconnect) {
  const keyspaces = {};
  const result = await this.cc.queryAsync(_selectAllKeyspacesV2, waitReconnect);
...
/** @override */
SchemaParserV2.prototype.getKeyspace = async function (name) {
...
/** @override */
SchemaParserV2.prototype.getMaterializedView = async function (keyspaceName, name, cache) {
...
/** @override */
SchemaParserV2.prototype._parseTableOrView = async function (tableInfo, tableRow, columnRows, indexRows, virtual) {
  const encoder = this.cc.getEncoder();
  const columnsKeyed = {};
  const partitionKeys = [];
  ...
  tableInfo.columns = await Promise.all(columnRows.map(async (row) => {
    const type = await encoder.parseTypeNameAsync(tableRow['keyspace_name'], row['type'], 0, null, this.udtResolver);
...
SchemaParserV2.prototype._parseAggregate = async function (row) {
  const encoder = this.cc.getEncoder();
  const aggregate = new Aggregate();
  aggregate.name = row['aggregate_name'];
  aggregate.keyspaceName = row['keyspace_name'];
  ...
  aggregate.argumentTypes = await Promise.all(aggregate.signature.map(name =>
    encoder.parseTypeNameAsync(row['keyspace_name'], name, 0, null, this.udtResolver)));

  aggregate.stateType = await encoder.parseTypeNameAsync(
    row['keyspace_name'], row['state_type'], 0, null, this.udtResolver);

  aggregate.returnType = await encoder.parseTypeNameAsync(
    row['keyspace_name'], row['return_type'], 0, null, this.udtResolver);

  return aggregate;
};

SchemaParserV2.prototype._parseFunction = async function (row) {
  const encoder = this.cc.getEncoder();
  const func = new SchemaFunction();
  func.name = row['function_name'];
  func.keyspaceName = row['keyspace_name'];
  ...
  func.argumentTypes = await Promise.all(func.signature.map(name =>
    encoder.parseTypeNameAsync(row['keyspace_name'], name, 0, null, this.udtResolver)));

  func.returnType = await encoder.parseTypeNameAsync(
    row['keyspace_name'], row['return_type'], 0, null, this.udtResolver);

  return func;
};

SchemaParserV2.prototype._parseUdt = async function (udtInfo, row) {
  const encoder = this.cc.getEncoder();
  const fieldTypes = row['field_types'];
  const keyspace = row['keyspace_name'];

  udtInfo.fields = await Promise.all(row['field_names'].map(async (name, i) => {
    const type = await encoder.parseTypeNameAsync(keyspace, fieldTypes[i], 0, null, this.udtResolver);
    return { name, type };
  }));
...
/** @override */
SchemaParserV3.prototype.getKeyspaces = async function (waitReconnect) {
  const keyspaces = {};
  ...
  await Promise.all(queries.map(async q => {
    ...
    try {
      result = await this.cc.queryAsync(q.query, waitReconnect);
    } catch (err) {
      if (q.virtual) {
...
/** @override */
SchemaParserV3.prototype.getKeyspace = async function (name) {
...
SchemaParserV3.prototype._getKeyspace = function (query, name, virtual) {
  try {
    const row = this._getFirstRow(format(query, name));
