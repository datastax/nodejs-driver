...
class SchemaParserV2 extends SchemaParser {
  ...
  _parseKeyspace(row, virtual) {
    ...
    return this._createKeyspace(row['keyspace_name'], row['durable_writes'], strategy, strategyOptions, virtual);
  }
  ...
  async _parseAggregate(row) {
    ...
    aggregate.argumentTypes = await Promise.all(aggregate.signature.map(name => encoder.parseTypeNameAsync(row['keyspace_name'], name, 0, null, this.udtResolver)));
    aggregate.stateType = await encoder.parseTypeNameAsync(row['keyspace_name'], row['state_type'], 0, null, this.udtResolver);
    aggregate.returnType = await encoder.parseTypeNameAsync(row['keyspace_name'], row['return_type'], 0, null, this.udtResolver);
    return aggregate;
  }
  ...
  async _parseFunction(row) {
    ...
    func.argumentTypes = await Promise.all(func.signature.map(name => encoder.parseTypeNameAsync(row['keyspace_name'], name, 0, null, this.udtResolver)));
    func.returnType = await encoder.parseTypeNameAsync(row['keyspace_name'], row['return_type'], 0, null, this.udtResolver);
    return func;
  }
