 ...
 * defaultMetrics.successfulResponses.on('increment', latency => specExecsCount++);
 */
class DefaultMetrics extends ClientMetrics {
  ...
  constructor() {
    ...
    /**
     * Emits events when a response message is obtained without any errors.
     * @type {EventEmitter}
     */
    this.successfulResponses = new EventEmitter();
  }
  ...
  onSuccessfulResponse(latency) {
    this.successfulResponses.emit('increment', latency);
  }
}
