...
class OperationState {
   ...
   */
  setRequestTimeout(info, defaultReadTimeout, address, onTimeout, onResponse) {
    if (this._state !== state.init) {
      // No need to set the timeout
      return;
    }
    const millis = info.getReadTimeout() !== undefined ? info.getReadTimeout() : defaultReadTimeout;
    if (!(millis > 0)) {
