 ...
 * @param {?String} [localDc] Deprecated: local datacenter name.  This value is
 * still used if set, but specifying localDataCenter in Client options should be
 * used instead.
...
DCAwareRoundRobinPolicy.prototype.init = function (client, hosts, callback) {
  ...
  // If localDc is unset, use value set in client options, if that is unset,
  // raise an error.
  if (this.localDc && !client.options.localDataCenter) {
    this.client.log('warning', 'Local data center \'' + this.localDc + '\' was provided as an argument to' +
      ' DCAwareRoundRobinPolicy. This is considered deprecated and will be removed in a future release. Instead,' +
      ' please specify local data center using \'localDataCenter\' in Client options.');
  }

  this.localDc = this.localDc || client.options.localDataCenter;
  if (!this.localDc) {
    return callback(new errors.DriverInternalError('\'localDataCenter\' is not defined in Client options'));
  }
  callback();
};
