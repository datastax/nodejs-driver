 ...
 * requests to different data cents.
...
function DseLoadBalancingPolicy(localDc, usedHostPerRemoteDc) {
  if (!localDc || typeof localDc === 'string') {
    this._childPolicy = new TokenAwarePolicy(new DCAwareRoundRobinPolicy(localDc, usedHostPerRemoteDc));
  }
  else if (localDc instanceof LoadBalancingPolicy) {
 ...
 * @override
 */
DseLoadBalancingPolicy.prototype.newQueryPlan = function (keyspace, executionOptions, callback) {
  const self = this;
  this._childPolicy.newQueryPlan(keyspace, executionOptions, function (err, iterator) {
    if (err) {
      return callback(err);
    }
    if (executionOptions && executionOptions.getPreferredHost()) {
      self._preferredHost = executionOptions.getPreferredHost();
      return callback(null, self._setFirst(self._preferredHost, iterator));
    }
...
DseLoadBalancingPolicy.prototype._setFirst = function (preferredHost, iterator) {
  let first = preferredHost;
  return ({
    next: function () {
      if (first) {
        const preferredHost = first;
        first = null;
