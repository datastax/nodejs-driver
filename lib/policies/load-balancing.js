 ...
 * @param {ExecutionInfo|null} info The information related to the execution of the request.
 * @param {Function} callback The function to be invoked with the error as first parameter and the host iterator as
 * second parameter.
 */
LoadBalancingPolicy.prototype.newQueryPlan = function (keyspace, info, callback) {
 ...
 * second parameter.
 */
RoundRobinPolicy.prototype.newQueryPlan = function (keyspace, info, callback) {
  if (!this.hosts) {
 ...
 * second parameter.
 */
DCAwareRoundRobinPolicy.prototype.newQueryPlan = function (keyspace, info, callback) {
  if (!this.hosts) {
    return callback(new Error('Load balancing policy not initialized'));
  }
  this.index += 1;
 ...
 * second parameter.
 */
TokenAwarePolicy.prototype.newQueryPlan = function (keyspace, info, callback) {
  let routingKey;
  if (info) {
  ...
  if (!routingKey || !replicas) {
    return this.childPolicy.newQueryPlan(keyspace, info, callback);
  }
  const iterator = new TokenAwareIterator(keyspace, info, replicas, this.childPolicy);
 ...
 * @ignore
 */
function TokenAwareIterator(keyspace, info, replicas, childPolicy) {
...
TokenAwareIterator.prototype.iterate = function (callback) {
  ...
  const self = this;
  this.childPolicy.newQueryPlan(this.keyspace, this.info, function (err, iterator) {
    if (err) {
      return callback(err);
    }
