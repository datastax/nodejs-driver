 ...
 * @returns {*}
 */
TokenAwarePolicy.prototype.newQueryPlan = function (queryOptions, callback) {
  var routingKey;
  if (queryOptions) {
  ...
  var replicas;
  if (routingKey) {
    replicas = this.client.getReplicas(routingKey);
  }
  if (!routingKey || !replicas) {
    return this.childPolicy.newQueryPlan(queryOptions, callback);
  }
  var iterator = new TokenAwareIterator(queryOptions, replicas, this.childPolicy);
 ...
 * @param queryOptions
 * @param {HostMap} replicas
 * @param childPolicy
 * @constructor
 */
function TokenAwareIterator(queryOptions, replicas, childPolicy) {
  this.replicas = replicas;
  this.replicasArray = replicas.slice(0);
  this.childPolicy = childPolicy;
  this.queryOptions = queryOptions;
  this.childHosts = [];
  this.remoteReplicas = [];
  this.replicaIndex = 0;
  this.childIterator = utils.arrayIterator(this.childHosts);
...
TokenAwareIterator.prototype.iterate = function (callback) {
  ...
  var self = this;
  this.childPolicy.newQueryPlan(this.queryOptions, function (err, iterator) {
    if (err) return callback(err);
    var item = iterator.next();
    while (!item.done) {
      self.childHosts.push(item.value);
      item = iterator.next();
    }
...
TokenAwareIterator.prototype.nextLocalReplica = function () {
  ...
  var host = this.replicasArray[this.replicaIndex++];
