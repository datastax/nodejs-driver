...
class PrepareHandler {
  ...
  static async getPreparedAsync(client, loadBalancing, query, keyspace) {
    const info = client.metadata.getPreparedInfo(keyspace, query);
    if (info.queryId) {
      return info;
    }
    ...
    const instance = new PrepareHandler(client, loadBalancing);
    return await instance._prepareAsync(info, query, keyspace);
  }
  ...
  static getPreparedMultiple(client, loadBalancing, queries, keyspace, callback) {
    ...
    utils.forEachOf(queries, function eachQuery(item, index, next) {
      ...
      PrepareHandler.getPrepared(client, loadBalancing, query, keyspace, function getPrepareCb(err, { queryId, meta }) {
        if (err) {
          return next(err);
        }

        result[index] = { query, params: utils.adaptNamedParamsPrepared(item.params, meta.columns), queryId, meta };
        next();
      });
  ...
  //TODO: Remove counterpart
  async _prepareAsync(info, query, keyspace) {
    info.preparing = true;
    let iterator;
    ...
    try {
      ...
      this._onPrepareError2(err, info, query);
    }

    return await this._prepareWithQueryPlanAsync(info, iterator, null, query, keyspace);
  }
   ...
   * @private
   */
  async _prepareWithQueryPlanAsync(info, iterator, triedHosts, query, keyspace) {
    //TODO: Review unnecessary assignment
    triedHosts = triedHosts || {};

    while (true) {
      //TODO: Make method public
      const host = RequestHandler._getNextHost(iterator, this._client.profileManager, triedHosts);

      if (host === null) {
        this._onPrepareError2(new errors.NoHostAvailableError(triedHosts), info, query);
      }
  ...
  _onPrepareError(err, host, triedHosts, info, iterator, query, keyspace) {
    if (host && (err.isSocketError || err instanceof errors.OperationTimedOutError)) {
      const self = this;
      triedHosts[host.address] = err;
  ...
  //TODO: Remove counterpart
  /**
   * Sets the event emitter state and throws an error and
   * @param {Error} err
   * @param {Object} info
   * @param {String} query
   * @private
   */
  _onPrepareError2(err, info, query) {
    info.preparing = false;
    err.query = query;
