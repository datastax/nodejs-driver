...
FrameReader.prototype.tryReadMetadata = function () {
  ...
  if (kind !== types.resultKind.rows) {
    throw new Error('Can not stream this kind of result');
  }
...
FrameStreamingParser.prototype.streamFrame = function (item) {
  var frame = this.frames[item.header.streamId];
  ...
  if (!frame.metadata) {
    ...
    var buffer = frame.chunk;
    if (frame.buffer) {
      buffer = Buffer.concat([frame.buffer, frame.chunk], frame.buffer.length + frame.chunk.length);
    }
    reader = new FrameReader(frame.header, buffer);
    frame.metadata = reader.tryReadMetadata();
    if (!frame.metadata) {
