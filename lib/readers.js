      ...
      for(var i = 0; i < rowCount; i++) {
        ...
        row.get = function(name) {
          ...
          if (typeof cellIndex === 'string') {
            cellIndex = meta.columns['_col_' + name];
          }
        ...
        for(var col = 0; col < meta.columns.length; col++ ) {
          ...
          var cellValue = this.convert(bytes, spec.type);
          row[col] = cellValue;
        }
        rows.push(row);
      }
    ...
    this.convert = function(bytes, type) {
      switch(type[0]) {
        ...
        case 0x000F: //Timeuuid
          //Copy the buffer and move on :)
          //https://github.com/datastax/csharp-driver/blob/master/Cassandra/TypeAdapters.cs
          var targetBuffer = new Buffer(bytes.length);
          bytes.copy(targetBuffer);
          return targetBuffer;
          break;
        case 0x0001: //Ascii
...
ResponseHandler.prototype.handle = function(data) {
  ...
  if(reader.opcode == types.opcodes.ready) {
    ...
    this.callback(null, reader.parseResult());
  } 
