...
class RequestExecution {
  ...
  _sendOnConnection() {
    ...
    this._operation =
      this._connection.sendStream(this._request, this._parent.requestOptions, function rCb(err, response, length) {
        ...
        if (response.schemaChange) {
          return self._parent.client.handleSchemaAgreementAndRefresh(
            self._connection, response.schemaChange, (agreement) => {
              if (self._cancelled) {
        ...
        if (response.meta && response.meta.newResultId && self._request.queryId) {
          ...
          const info = self._parent.client.metadata.getPreparedById(self._request.queryId);
  ...
  _trackResponse(startTime, err, length) {
    ...
    if (err) {
      tracker.onError(this._host, query, parameters, this._parent.requestOptions, requestLength, err, latency);
    } else {
      tracker.onSuccess(this._host, query, parameters, this._parent.requestOptions, requestLength, length, latency);
    }
  }
