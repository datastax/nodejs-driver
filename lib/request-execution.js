...
class RequestExecution {
  ...
  _trackResponse(startTime, errorCode, err, length) {
    ...
    RequestExecution._invokeMetricsHandler(errorCode, metrics, err, latency);
    ...
    switch (errorCode) {
      ...
      case errorCodes.serverErrorOverloaded:
        if (operationInfo.options.isIdempotent) {
          return this._parent.retryPolicy.onRequestError(operationInfo, this._request.consistency, err);
        }
