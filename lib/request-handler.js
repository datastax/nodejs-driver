...
class RequestHandler {
   ...
   */
  //TODO: Rename
  static async borrowNextConnectionAsync(iterator, triedHosts, profileManager, keyspace) {
    ...
    while (true) {
      ...
      try {
        connection = host.borrowSync(keyspace);
        if (connection === null) {
          connection = await host.borrowConnectionAsync(keyspace, null);
        }
   ...
   * @returns {Promise<{host, connection}>}
   */
  getNextConnection() {
    return RequestHandler.borrowNextConnectionAsync(
      this._hostIterator, this.triedHosts, this.client.profileManager, this.client.keyspace);
  }
  ...
  async _startNewExecution(isSpecExec) {
    ...
    try {
      ({ host, connection } = await this.getNextConnection());
    } catch (err) {
