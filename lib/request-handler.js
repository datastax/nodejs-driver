...
RequestHandler.prototype.sendOnConnection = function (request, options, callback) {
  ...
  this.connection.sendStream(request, options, function (err, response) {
    ...
    self.host.setUp();
    if (response) {
      var result = new ResultSet(response, self.host.address, self.triedHosts, self.request.consistency);
...
RequestHandler.prototype.prepareAndRetry = function (queryId, callback) {
  ...
  if (this.request instanceof requests.BatchRequest) {
    ...
    //We need to prepare all the queries in the batch in the current host
    var queryStrings = new Array(this.request.queries.length);
    var singleCallbacks = new Array(this.request.queries.length);
    this.request.queries.forEach(function (item, i) {
      queryStrings[i] = item.query;
      singleCallbacks[i] = function (err, response) {
        if (err) return; //let the error be handle at general callback level
        if (!Buffer.isBuffer(response.id) ||
