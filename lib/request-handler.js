...
function RequestHandler(client, options) {
  ...
  var profile = client.profileManager.getDefault();
  this.loadBalancingPolicy = profile.loadBalancing;
  this.retryPolicy = profile.retry;
...
RequestHandler.prototype.send = function (request, options, callback) {
  if (this.request === null) {
    ...
    var profile = options.executionProfile || this.profileManager.getDefault();
    this.retryPolicy = options.retry || profile.retry;
    this.loadBalancingPolicy = profile.loadBalancing;
  }
