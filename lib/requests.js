...
ExecuteRequest.prototype.writeQueryParameters = function (frameWriter, encoder) {
  ...
  if (types.protocolVersion.supportsPaging(this.version)) {
    ...
    if (types.protocolVersion.uses4BytesQueryFlags(this.version)) {
      if (types.protocolVersion.supportsContinuousPaging(this.version) && this.options.continuousPaging) {
...
CancelRequest.prototype.write = function () {
  var frameWriter = new FrameWriter(types.opcodes.cancel);
