...
const ExecutionInfo = require('./execution-info').ExecutionInfo;
...
class ExecuteRequest extends Request {
   ...
   * @param meta
   */
  constructor(query, queryId, params, info, meta) {
    super();
    ...
    this.params = params;
    this.meta = meta;
    this.info = info || ExecutionInfo.empty();
    this.consistency = this.info.getConsistency() || types.consistencies.one;
  ...
  write(encoder, streamId) {
    ...
    let headerFlags = this.info.getIsQueryTracing() ? types.frameFlags.tracing : 0;
    if (this.info.getCustomPayload()) {
      //The body may contain the custom payload
      headerFlags |= types.frameFlags.customPayload;
      frameWriter.writeCustomPayload(this.info.getCustomPayload());
    }
  ...
  writeQueryParameters(frameWriter, encoder) {
    ...
    const timestamp = this.info.getOrGenerateTimestamp();

    if (types.protocolVersion.supportsPaging(encoder.protocolVersion)) {
      flags |= (this.params && this.params.length) ? queryFlag.values : 0;
      flags |= (this.info.getFetchSize() > 0) ? queryFlag.pageSize : 0;
      flags |= this.info.getPageState() ? queryFlag.withPagingState : 0;
      flags |= this.info.getSerialConsistency() ? queryFlag.withSerialConsistency : 0;
    ...
    if (flags & queryFlag.pageSize) {
      frameWriter.writeInt(this.info.getFetchSize());
    }
    if (flags & queryFlag.withPagingState) {
      frameWriter.writeBytes(this.info.getPageState());
    }
    if (flags & queryFlag.withSerialConsistency) {
      frameWriter.writeShort(this.info.getSerialConsistency());
    }
...
class QueryRequest extends ExecuteRequest {
   ...
   */
  constructor(query, params, info, namedParameters) {
    super(query, null, params, info, null);
    this.hints = this.info.getHints() || utils.emptyArray;
...
class BatchRequest extends Request {
   ...
   * @param {ExecutionInfo} info
   */
  constructor(queries, info) {
    super();
    this.queries = queries;
    this.info = info;
    this.hints = info.getHints() || utils.emptyArray;
    this.type = batchType.logged;

    if (info.getIsBatchCounter()) {
      this.type = batchType.counter;
    } else if (!info.getIsBatchLogged()) {
  ...
  write(encoder, streamId) {
    ...
    frameWriter.writeShort(this.info.getConsistency());

    if (types.protocolVersion.supportsTimestamp(encoder.protocolVersion)) {
      // Batch flags
      let flags = this.info.getSerialConsistency() ? batchFlag.withSerialConsistency : 0;
      const timestamp = this.info.getOrGenerateTimestamp();
  ...
  clone() {
    return new BatchRequest(this.queries, this.info);
  }
}
