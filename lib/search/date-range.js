...
function writeDate(date, buffer, offset) {
  var long = Long.fromNumber(date.getTime());
...
function readDate(buffer, offset) {
  var long = new Long(buffer.readInt32BE(offset+4), buffer.readInt32BE(offset));
 ...
 * defined in the [dateRangePrecision]{@link module:search~dateRangePrecision} member.
 * @constructor
 * @memberOf module:search
 */
function DateRangeBound(date, precision) {
   ...
   * [dateRangePrecision]{@link module:search~dateRangePrecision} member.
   * @type {Number}
   */
...
DateRangeBound.toLowerBound = function (bound) {
  ...
  var rounded = new Date(bound.date.getTime());
