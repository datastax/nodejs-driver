...
Parser.prototype._transform = function (item, encoding, callback) {
  ...
  if (item.frameEnded) {
    ...
    this.emitItem(item.header, {frameEnded: true});
  }
};

/**
 * Pushes the item with the header and the provided props
 */
Parser.prototype.emitItem = function (header, extendedProps) {
  this.push(utils.extend({header: header}, extendedProps));
};

Parser.prototype.parseBody = function (item) {
  var frameInfo = this.frames[item.header.streamId];
  if (!frameInfo) {
    frameInfo = this.frames[item.header.streamId] = {header: item.header};
  }
  if (item.frameEnded) {
  ...
  switch (item.header.opcode) {
    case types.opcodes.ready:
      return this.emitItem(frameInfo.header, {ready: true});
    case types.opcodes.authenticate:
      return this.emitItem(frameInfo.header, {mustAuthenticate: true});
    case types.opcodes.error:
      return this.parseError(frameInfo, reader);
    case types.opcodes.result:
      return this.parseResult(frameInfo, item, reader);
    default:
      return this.emitItem(frameInfo.header, {error: new Error('Received invalid opcode: ' + item.header.opcode)});
  }
};
...
Parser.prototype.parseError = function (frameInfo, reader) {
  try {
    this.emitItem(frameInfo.header, {error: reader.readError()});
  }
  catch (e) {
 ...
 * @param item header and body chunk
 * @param {FrameReader} reader
 */
Parser.prototype.parseResult = function (frameInfo, item, reader) {
  ...
  if (frameInfo.kind !== types.resultKind.rows) {
    return this.emitItem(frameInfo.header, {id: frameInfo.preparedId, meta: frameInfo.meta});
  }
...
Parser.prototype.streamRows = function (frameInfo, reader) {
  ...
  if (frameInfo.rowLength === 0) {
    return this.emitItem(frameInfo.header, {result: {rows: []}});
  }
    ...
    for(var j = 0; j < meta.columns.length; j++ ) {
      ...
      if (col.name !== frameInfo.streamingColumn) {
        ...
        if (j === meta.columns.length -1) {
          ...
          this.emitItem(frameInfo.header, {
            row: row,
            meta: frameInfo.meta,
            streamRows: frameInfo.streamRows,
...
Parser.prototype.setStreaming = function (id, streamField) {
  if (this.frames[id.toString()]) {
    throw new types.DriverError('There was already state for this frame');
  }
  this.frames[id.toString()] = {streamRows: true, streamField: streamField};
};
...
ResultEmitter.prototype.each = function (item) {
  ...
  if (item.streamRows) {
    return this.emit('row', item.header, item.row, item.fieldStream);
  }
  if (item.row) {
    //its a row but needs to be buffered to an array of rows
