...
Parser.prototype.parseResult = function (frameInfo, reader) {
  ...
  try {
    if (frameInfo.header.isTracing() && !frameInfo.traceId) {
      reader.checkOffset(16);
      frameInfo.traceId = new types.Uuid(utils.copyBuffer(reader.read(16)));
    }
    if (!frameInfo.meta) {
...
Parser.prototype.parseRows = function (frameInfo, reader) {
  ...
  if (frameInfo.rowLength === 0) {
    return this.emitItem(frameInfo, { result: { rows: utils.emptyArray, meta: frameInfo.meta, traceId: frameInfo.traceId}});
  }
  ...
  for (var i = frameInfo.rowIndex; i < frameInfo.rowLength; i++) {
    ...
    this.emitItem(frameInfo, {
      row: new types.Row(this.encoder, meta, rowValues),
      meta: frameInfo.meta,
      byRow: frameInfo.byRow,
      length: frameInfo.rowLength,
      traceId: frameInfo.traceId
    });
  }
};
...
ResultEmitter.prototype.each = function (item) {
  ...
  if (item.byRow) {
    ...
    return this.emit('row', item.header, item.row, item.meta, item.length, item.traceId);
  }
  if (item.row) {
...
ResultEmitter.prototype.bufferAndEmit = function (item) {
  ...
  if (rows.length === item.length) {
    this.emit('result', item.header, null, { rows: rows, meta: item.meta, traceId: item.traceId});
