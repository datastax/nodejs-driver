...
Parser.prototype.parseResult = function (frameInfo, reader) {
  ...
  // Its a `Rows` result
  if (frameInfo.byRow && frameInfo.meta.lastContinuousPage) {
    // This frame is the last of series of frames that reused the same streamId
    this.push({ header: frameInfo.header, lastContinuousPage: true });
  }
...
Parser.prototype.parseRows = function (frameInfo, reader) {
  ...
  if (frameInfo.rowLength === 0 && !frameInfo.byRow) {
    return this.push({
      header: frameInfo.header,
...
ResultEmitter.prototype.each = function (item) {
  ...
  if (item.frameEnded) {
    return this.emit('frameEnded', item.header.streamId);
  }
  ...
  if (item.byRow) {
    ...
    return this.emit('row', item.header, item.row, item.length);
  }
  if (item.row) {
