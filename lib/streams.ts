...
class Parser extends Transform{
   ...
   */
  parseBody(frameInfo, item: { header: FrameHeader; chunk: Buffer; offset: number; }) {
   ...
   * @returns {Boolean}
   */
  handleFrameBuffers(frameInfo, item: { header: FrameHeader; chunk: Buffer; offset: number; }): boolean {
   ...
   * @param {{header: FrameHeader, chunk: Buffer, offset: Number}} item
   */
  addFrameBuffer(frameInfo, item: { header: FrameHeader; chunk: Buffer; offset: number; }) {
    if (!frameInfo.buffers) {
   ...
   * @param {{header: FrameHeader, chunk: Buffer, offset: Number}} item
   */
  getFrameBuffer(frameInfo, item: { header: FrameHeader; chunk: Buffer; offset: number; }) {
    frameInfo.buffers.push(item.chunk);
   ...
   */
  parseResult(frameInfo, reader: FrameReader) {
    let result;
      ...
      if (!frameInfo.meta) {
        ...
        switch (frameInfo.kind) {
          ...
          case types.resultKind.schemaChange:
            ...
            break;
          default:
            throw new errors.DriverInternalError('Unexpected result kind: ' + frameInfo.kind);
        }
      }
    }
    catch (e) {
   ...
   * @param {FrameReader} reader
   */
  parseRows(frameInfo, reader: FrameReader) {
   ...
   */
  handleParsingError(e: Error, frameInfo, reader: FrameReader, originalOffset?: number, rowIndex?: number) {
   ...
   */
  bufferResultCell(frameInfo, reader: FrameReader, originalOffset: number, rowIndex: number, expectedLength: number) {
