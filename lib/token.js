...
class Murmur3Token extends Token {
  ...
  toString() {
    // Use MutableLong's toString rather than Tokenizer's as it
    // is in a format that matches C*'s.
...
class TokenRange {
   ...
   */
  isWrappedAround() {
    return this.start.compare(this.end) > 0 && !this.start.equals(this._tokenizer.minToken());
  }
  ...
  contains(token) {
    ...
    if (this.end.equals(minToken)) {
      ...
      return token.compare(this.start);
    }

    const isAfterStart = token.compare(this.start) > 0;
    const isBeforeEnd = token.compareTo(this.end) <= 0;
