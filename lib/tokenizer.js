...
class ByteOrderedTokenizer extends Tokenizer {
  ...
  hash(value) {
    ...
    return new token.ByteOrderedToken(value.slice(0, zeroIndex), this);
  }
  ...
  _toNumber(buffer, significantBytes) {
    ...
    return Integer.fromBuffer(buffer);
  }
  ...
  split(start, end, numberOfSplits) {
    ...
    return values.map(v => new token.ByteOrderedToken(this._toBuffer(v, significantBytes), this));
  }
}
