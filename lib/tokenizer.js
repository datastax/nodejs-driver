...
class Murmur3Tokenizer extends Tokenizer {
  ...
  split(start, end, numberOfSplits) {
    ...
    return values.map(v => this.parse(v.toString(), this));
  }
...
class RandomTokenizer extends Tokenizer {
  ...
  maxValue() {
    if (!this._maxValue) {
      this._maxValue = Integer.fromInt(Math.pow(2, 127));
    }
    return this._maxValue;
  }
  ...
  maxToken() {
    if (!this._maxToken) {
      this._maxToken = new token.RandomToken(this.maxValue(), this);
    }
    return this._maxToken;
  }
  ...
  ringLength() {
    if (!this._ringLength) {
      this._ringLength = new Integer(Math.pow(2, 127) + 1);
    }
    return this._ringLength;
  }
  ...
  split(start, end, numberOfSplits) {
    ...
    const values = this.splitBase(startVal, range, this.maxValue(), this.ringLength(), numberOfSplits);
    return values.map(v => new token.RandomToken(v, this));
  }
...
class ByteOrderedTokenizer extends Tokenizer {
  ...
  _toNumber(buffer, significantBytes) {
    ...
    // TODO: Get MutableLong from buffer.
    return MutableLong.fromNumber(0);
  }

  _toBuffer(number, significantBytes) {
    // TODO: Get Buffer from Long.
  }
  ...
  split(start, end, numberOfSplits) {
    ...
    const longNumberOfSplits = MutableLong.fromNumber(numberOfSplits);
    ...
    if (tokenOrder < 0) {
      ...
      while (true) {
        ...
        if (addedBytes === 4 || range.compare(longNumberOfSplits) >= 0) {
          break;
        }
        significantBytes += 1;
        addedBytes += 1;
      ...
      while (true) {
        startVal = this._toNumber(start.getValue(), significantBytes);
        endVal = this._toNumber(end.getValue(), significantBytes);
        ringLength = MutableLong.fromNumber(Math.pow(2, significantBytes * 8));
        ringEnd = MutableLong.clone(ringLength).subtract(MutableLong.one);
        range = endVal.clone().subtract(startVal).add(ringLength);
        if (addedBytes === 4 || range.compare(longNumberOfSplits) >= 0) {
          break;
        }
        significantBytes += 1;
    ...
    const values = this.splitBase(MutableLong.fromNumber(startVal), 
      MutableLong.fromNumber(range), MutableLong.fromNumber(ringEnd), MutableLong.fromNumber(ringLength), numberOfSplits);
    return values.map(v => new token.ByteOrderedToken(this._toBuffer(v.toNumber(), significantBytes), this));
  }
}
