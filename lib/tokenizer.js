...
class Murmur3Tokenizer extends Tokenizer {
  ...
  split(start, end, numberOfSplits) {
    ...
    const startVal = Integer.fromString(start._value.toString());
    const endVal = Integer.fromString(end._value.toString());
...
class RandomTokenizer extends Tokenizer {
  ...
  split(start, end, numberOfSplits) {
    ...
    const startVal = start._value;
    const endVal = end._value;
...
class ByteOrderedTokenizer extends Tokenizer {
  ...
  split(start, end, numberOfSplits) {
    ...
    let significantBytes = Math.max(start._value.length, end._value.length);
    if (tokenOrder < 0) {
      let addedBytes = 0;
      while (true) {
        startVal = this._toNumber(start._value, significantBytes);
        endVal = this._toNumber(end._value, significantBytes);
