...
class RequestLogger extends RequestTracker {
  ...
  onSuccess(host, query, parameters, info, requestLength, responseLength, latency) {
    if (this._options.slowThreshold > 0 && toMillis(latency) > this._options.slowThreshold) {
      this._logSlow(host, query, parameters, info, requestLength, responseLength, latency);
    }
    else if (this._options.requestSizeThreshold > 0 && requestLength > this._options.requestSizeThreshold) {
      this._logLargeRequest(host, query, parameters, info, requestLength, responseLength, latency);
    }
    else if (this.logNormalRequests) {
      this._logNormalRequest(host, query, parameters, info, requestLength, responseLength, latency);
    }
  }
  ...
  onError(host, query, parameters, info, requestLength, err, latency) {
    if (this._options.requestSizeThreshold > 0 && requestLength > this._options.requestSizeThreshold) {
      this._logLargeErrorRequest(host, query, parameters, info, requestLength, err, latency);
    }
    else if (this.logErroredRequests) {
      this._logErrorRequest(host, query, parameters, info, requestLength, err, latency);
    }
  }

  _logSlow(host, query, parameters, info, requestLength, responseLength, latency) {
    const message = format('[%s] Slow request, took %d ms (%s): %s', host.address, Math.floor(toMillis(latency)),
      getPayloadSizes(requestLength, responseLength), getStatementInfo(query, parameters, info, this._options));
    this.emitter.emit('slow', message);
  }

  _logLargeRequest(host, query, parameters, info, requestLength, responseLength, latency) {
    const message = format('[%s] Request exceeded length, %s (took %d ms): %s', host.address,
      ...
      getStatementInfo(query, parameters, info, this._options));
    this.emitter.emit('large', message);
  }

  _logNormalRequest(host, query, parameters, info, requestLength, responseLength, latency) {
  ...
  _logLargeErrorRequest(host, query, parameters, info, requestLength, err, latency) {
    ...
    const message = format('[%s] Request exceeded length and execution failed, %s (took %d ms): %s; error: %s',
      ...
      getStatementInfo(query, parameters, info, this._options), err.stack.substr(0, maxStackTraceLength));

    // Use 'large' event and not 'failure' as this log is caused by exceeded length
    this.emitter.emit('large', message);
  }

  _logErrorRequest(host, query, parameters, info, requestLength, err, latency) {
    const maxStackTraceLength = this._options.messageMaxErrorStackTraceLength || defaultMaxErrorStackTraceLength;
...
function getStatementInfo(query, parameters, info, options) {
  ...
  if (Array.isArray(query)) {
    return getBatchStatementInfo(query, info, maxQueryLength, maxParameterLength);
  }
...
function getBatchStatementInfo(queries, info, maxQueryLength, maxParameterLength) {
  // This part of the message is not accounted for in "maxQueryLength"
  let message = (info.getIsBatchLogged() ? 'LOGGED ' : '') + 'BATCH w/ ' + queries.length +
    (!info.getIsPrepared() ? ' not prepared' : '') + ' queries (';
