...
class RequestLogger extends RequestTracker {
  ...
  onSuccess(host, query, parameters, queryOptions, requestLength, responseLength, latency) {
    if (this._options.slowThreshold > 0 && toMillis(latency) > this._options.slowThreshold) {
      this._logSlow(host, query, parameters, queryOptions, requestLength, responseLength, latency);
    }
    else if (this._options.requestSizeThreshold > 0 && requestLength > this._options.requestSizeThreshold) {
      this._logLargeRequest(host, query, parameters, queryOptions, requestLength, responseLength, latency);
    }
    else if (this.logNormalRequests) {
      this._logNormalRequest(host, query, parameters, queryOptions, requestLength, responseLength, latency);
    }
  }
  ...
  onError(host, query, parameters, queryOptions, requestLength, err, latency) {
    if (this._options.requestSizeThreshold > 0 && requestLength > this._options.requestSizeThreshold) {
      this._logLargeErrorRequest(host, query, parameters, queryOptions, requestLength, err, latency);
    }
    else if (this.logErroredRequests) {
      this._logErrorRequest(host, query, parameters, queryOptions, requestLength, err, latency);
    }
  }

  _logSlow(host, query, parameters, queryOptions, requestLength, responseLength, latency) {
    const message = format('[%s] Slow request, took %d ms (%s): %s', host.address, ~~toMillis(latency),
      getPayloadSizes(requestLength, responseLength), getStatementInfo(query, parameters, queryOptions, this._options));
    this.emitter.emit('slow', message);
  }

  _logLargeRequest(host, query, parameters, queryOptions, requestLength, responseLength, latency) {
    const message = format('[%s] Request exceeded length, %s (took %d ms): %s', host.address,
      ...
      getStatementInfo(query, parameters, queryOptions, this._options));
    this.emitter.emit('large', message);
  }

  _logNormalRequest(host, query, parameters, queryOptions, requestLength, responseLength, latency) {
  ...
  _logLargeErrorRequest(host, query, parameters, queryOptions, requestLength, err, latency) {
    ...
    const message = format('[%s] Request exceeded length and execution failed, %s (took %d ms): %s; error: %s',
      ...
      getStatementInfo(query, parameters, queryOptions, this._options), err.stack.substr(0, maxStackTraceLength));

    // Use 'large' event and not 'failure' as this log is caused by exceeded length
    this.emitter.emit('large', message);
  }

  _logErrorRequest(host, query, parameters, queryOptions, requestLength, err, latency) {
    const maxStackTraceLength = this._options.messageMaxErrorStackTraceLength || defaultMaxErrorStackTraceLength;
...
function getStatementInfo(query, parameters, queryOptions, options) {
  ...
  if (Array.isArray(query)) {
    return getBatchStatementInfo(query, queryOptions, maxQueryLength, maxParameterLength);
  }
...
function getBatchStatementInfo(queries, queryOptions, maxQueryLength, maxParameterLength) {
  // This part of the message is not accounted for in "maxQueryLength"
  let message = (queryOptions.logged ? 'LOGGED ' : '') + 'BATCH w/ ' + queries.length +
    (!queryOptions.prepare ? ' not prepared' : '') + ' queries (';
