...
var typeEncoder = (function(){
  ...
  function encodeArray(value, type) {
    ...
    var arrayLengthBuffer = new Buffer(2);
    arrayLengthBuffer.writeUInt16BE(value.length, 0);
    var parts = [];
    parts.push(arrayLengthBuffer);
    for (var i=0;i<value.length;i++) {
      var bytes = encode(value[0]);
      var itemLengthBuffer = new Buffer(2);
      itemLengthBuffer.writeUInt16BE(bytes.length, 0);
      //include item byte length
      parts.push(itemLengthBuffer);
      //include item
      parts.push(bytes);
    }
