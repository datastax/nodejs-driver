...
var dataTypes = {
  ...
  getByName:  function(name) {
    ...
    var typeInfo = { code: null, info: null};
    if (name.indexOf('<')) {
      var listMatches = /^(list|set)<(\w+)>$/.exec(name);
      if (listMatches) {
        name = listMatches[1];
        typeInfo.info = listMatches[2];
      }
      else {
        var mapMatches = /^(map)< *(\w+) *, *(\w+)>$/.exec(name);
        if (mapMatches) {
          name = mapMatches[1];
          typeInfo.info = [mapMatches[2], mapMatches[3]];
        }
      }
    }
    typeInfo.code = this[name];
