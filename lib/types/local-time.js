...
LocalTime.fromString = function (value) {
  ...
  var nanos = 0;
  if (parts.length === 3) {
    var secParts = parts[3].split('.');
    millis += parseInt(secParts[0], 10) * 1000;
  }
  return LocalTime.fromMilliseconds(millis, nanos);
};
 ...
 * @param {Date|Number} date Milliseconds expressed in Date or in Number.
 * @param {Number} [nanoseconds] A Number from 0 to 999,999, representing the time nanosecond portion.
 * @returns {LocalTime}
 */
 ...
 * @param {Number} [nanoseconds] A Number from 0 to 999,999, representing the time nanosecond portion.
 * If not provided, it will be a monotonically increasing value.
 * @returns {LocalTime}
 */
...
LocalTime.prototype.toString = function () {
  if (!this._stringCache) {
    ...
    //faster modulo
    //total nanos
    values[3] = this.value.subtract(secs.multiply(nanoSecInSec)).toNumber();
    //seconds
    values[2] = secs.toNumber();
    if (values[2] >= 60) {
      //minutes
      values[1] = Math.floor(values[2] / 60);
      values[2] = values[2] % 60;
    }
    if (values[1] >= 60) {
      //hours
      values[0] = Math.floor(values[1] / 60);
      values[1] = values[1] % 60;
    }
    this._stringCache =  formatTime(values);
  }
