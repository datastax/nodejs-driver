 ...
 * @return {number}
 */
MutableLong.compare = function (other) {
...
MutableLong.prototype.xor = function (other) {
  for (var i = 0; i < 4; i++) {
    if ((this._arr[i] >>> 0) > 0xffff) {
      throw new Error('failed at index ' + i + ' for value: ' + (this._arr[i] >>> 0).toString(16));
    }
    if ((other._arr[i] >>> 0) > 0xffff) {
      throw new Error('failed at index ' + i + ' for value: ' + (other._arr[i] >>> 0).toString(16));
    }
  }
  this._arr[0] ^= other._arr[0];
  this._arr[1] ^= other._arr[1];
