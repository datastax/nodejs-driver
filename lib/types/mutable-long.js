...
/**
 * A mutable long based on 4 int16 values.
 * @constructor
 */
function MutableLong(b00, b16, b32, b48) {
  this._arr = [ b00 | 0, b16 | 0, b32 | 0, b48 | 0 ];
}

MutableLong.fromNumber = function (value) {
...
MutableLong.fromBits = function (low32Bits, high32Bits) {
  return new MutableLong(low32Bits & 0xffff, low32Bits >>> 16, high32Bits & 0xffff, high32Bits >>> 16);
};

MutableLong.prototype.getShort = function (index) {
...
MutableLong.prototype.shiftLeft = function (numBits) {
  ...
  if (numBits < 16) {
    ...
    return this._shiftLeftBits(numBits);
  }
  if (numBits % 16 === 0) {
    // Shift entire int16 values
    return this._shiftLeftPosition(numBits / 16);
  }
  // Move blocks first
  this._shiftLeftPosition(Math.floor(numBits / 16));
  // Shift bits
  return this._shiftLeftBits(numBits % 16);
};
...
MutableLong.prototype.shiftRightUnsigned = function (numBits) {
  ...
  var pos = numBits % 16;
  var remainingBits = Math.floor(numBits / 16);
  if (pos > 0) {
  ...
  if (remainingBits > 0) {
    this._arr[0] = (this._arr[0] >>> remainingBits) | (this._arr[1] << (16 - remainingBits));
    this._arr[1] = (this._arr[1] >>> remainingBits) | (this._arr[2] << (16 - remainingBits));
    this._arr[2] = (this._arr[2] >>> remainingBits) | (this._arr[3] << (16 - remainingBits));
    this._arr[3] = (this._arr[3] >>> remainingBits) | (this._arr[4] << (16 - remainingBits));
  }
  return this;
};
...
/**
 * Shifts left whole blocks of shorts.
 * @param {Number} pos An integer representing the amount of positions.
 * @return {MutableLong}
 * @private
 */
MutableLong.prototype._shiftLeftPosition = function (pos) {
  this._arr[3] = this._arr[3 - pos];
  this._arr[2] = pos > 2 ? 0 : this._arr[3 - pos];
...
/**
 * Shifts left up to 16 bits.
 * @param numBits
 * @return {MutableLong}
 * @private
 */
MutableLong.prototype._shiftLeftBits = function (numBits) {
  this._arr[3] = ((this._arr[3] << numBits) | (this._arr[2] >>> (16 - numBits))) & 0xffff;
  this._arr[2] = ((this._arr[2] << numBits) | (this._arr[1] >>> (16 - numBits))) & 0xffff;
  this._arr[1] = ((this._arr[1] << numBits) | (this._arr[0] >>> (16 - numBits))) & 0xffff;
  this._arr[0] = (this._arr[0] << numBits) & 0xffff;
  return this;
};
...
MutableLong.prototype.toImmutable = function () {
  return Long.fromBits((this._arr[1] << 16) | this._arr[0], (this._arr[3] << 16) | this._arr[2], false);
};
