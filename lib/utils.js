...
/**
 * Converts an array of column metadata into hinted type info
 * @param {Array} columns
 * @returns {Array} An array of typeInfo
 */
function toHint(columns) {
  var hint = [];
  for (var i = 0; i < columns.length; i++) {
    ...
    if (util.isArray(subtypes) && util.isArray(subtypes[0])) {
      subtypes = toHint(subtypes);
    }
    hint.push({type: typeOption[0], subtypes: subtypes})
  }
  return hint;
}
...
function adaptNamedParams(params, columns) {
  ...
  for (var i = 0; i < columns.length; i++) {
    ...
    if (!params.hasOwnProperty(name)) {
      throw new Error(util.format('Parameter "%s" not defined', name));
    }
