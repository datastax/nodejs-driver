...
var queryParser = (function() {
  ...
  function quote(x) {
    x = x.replace("'", "''"); // escape strings with double single-quotes
    return "'" + x + "'";
  }
  ...
  function encodeParam(x) {
    ...
    // Arrays (assumed to be lists)
    if(x instanceof Array) {
      return stringifyArray(x);
    }
    if (typeof x === 'string') {
      // UUIDs are strings, but shouldn't be quoted
      if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(x)){
        return x;
      }else{
        return quote(x);
      }
    ...
    if (typeof x === 'object'){
      switch (guessCollectionType(x)) {
        ...
        break;
        default:
          throw new QueryParserError('guessCollectionType() is broken! Should always return "map", "list" or "set"');
      }
    }
  ...
  function guessCollectionType(obj){
    ...
    if (isMap){
      ...
      if (typeof obj.push === "function"){ // Sketchy type checking...
        return "list";
      }else{
        return "set";
      }
