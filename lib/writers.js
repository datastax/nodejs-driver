 ...
 * @constructor
 */
function BatchWriter(queries, options, consistency) {
...
BatchWriter.prototype.write = function () {
  if (!this.queries || !(this.queries.length > 0)) {
    throw new types.DriverError(util.format('Invalid queries provided %s', this.queries));
  }
  ...
  this.queries.forEach(function (item, index) {
    ...
    if (!query) {
      throw new types.DriverError(util.format('Invalid query at index %d', index));
    }
