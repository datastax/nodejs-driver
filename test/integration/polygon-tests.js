...
vdescribe('5.0', 'Polygon', function () {
  ...
  describe('with collections, tuples and udts', function () {
    ...
    [0, 1].forEach(function (prepare) {
      ...
      it(util.format('should create and retrieve polygons in a tuple for %s queries', name), function (done) {
        var client = new cassandra.Client(helper.getOptions());
        ...
        async.series([
          client.connect.bind(client),
          function (next) {
            client.execute(insertQuery, [id, tuple], {prepare: prepare, hints: [null, "tuple<int,'PolygonType'>"]}, function (err) {
              assert.ifError(err);
              client.execute(selectQuery, [id], {prepare: prepare}, function (err, result) {
                assert.ifError(err);
                var row = result.first();
